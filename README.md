...
firestore.rules    the access model (public answers, exact aggs,
                   member-only groups, sealed duels) — 161 emulator tests
...
- `npm run test:rules` — 161 security-rules tests (Firestore + Storage)
...
- `npm run check:appcheck` — every callable either demands App Check
  ...
  nine that cannot are the operator and moderator instruments, gated on
