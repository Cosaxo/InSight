...
| 6 | `research/axiom-theory/paper-06-genome-x-molecular-stream.md` | A stream from the body, beside the genome: a passive wearable's hourly multi-analyte channel as the body axis's measured channel, read as trends and never as values, crossed with the genome and every repeated measure; resilience as a response curve to a fixed challenge and aging as that curve's slope across years; the genetics of curve parameters and carriers as the nearest thing to a human knockout; the metabolic shadow of an immune event; three uses ranked by what the hardware delivers, with vector dosing rescoped to personal reference ranges; the overlap of stream and genome as the metric every genetic claim is gated on; two cheap experiments that decide the program's shape, and the lifts that cost decisions rather than engineering |
