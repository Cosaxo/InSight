...
| 12 | `research/axiom-theory/paper-12-where-the-join-runs.md` | Where the join runs: the genome crossings as computations where the file must not move, so only a summary leaves the device and which summaries suffice decides what is knowable — not how fast, but whether. A coupling's sufficient statistic that is a sum over people runs by each device contributing its own attested term against a phenotype public on the other side, which is the easy case wearing a hard case's reputation; an unattested device summary is a rumour the aggregate cannot use; every released coupling spends an output-privacy budget and an unbounded bank proposes couplings without end, so the budget is the binding resource and a policy allocates it. Kinship is pairwise and needs its own primitive at a higher price. And the join the learned representation needs is the one this channel cannot make — a per-person cross-axis sequence including the genome — so the teacher consumes a device-computed attested summary whose adequacy against the file is measured once in an enclave, never in the field |
...
| G4 | `research/general-theory/paper-g4-two-moves.md` | Two levers: a frame in which a research programme's knowledge of a population's coupling structure moves with what the observations carry and what the mechanism extracts, stated as a frame rather than a theorem proved complete, since a programme is defined as making decisions of those two kinds. The measurable half is real — a source's value on the evaluation stream is a number a belief cannot fake, so disagreement about what matters, given a fixed target, becomes disagreement about which source to fund and is settled by the ledger — and the unmeasurable half is named: the target is a theory, the candidate set's omissions are as invisible as any, identification is a state the value ledger cannot see so the most valuable observation can register near zero, and Goodhart is the failure the evaluation stream does not catch. The source-driven asymmetry holds only for a fixed target and class and a growable budget. Theory relocates to four written objects rather than disappearing; the transfer is stated for programmes measuring persons, agents and learned systems |
