...
describe("storage: the profile photo (D178)", () => {
  ...
  it("but not by the signed-out world THROUGH THE SDK — which is not how the app draws it", async () => {
    ...
    // the thing it was read as promising, and no assertion in an emulator
    // can: the token is a bearer credential and the served audience is
    // anyone holding the link.
    //
    // Measured 2026-09-10 against the storage emulator: the tokenless
    // address is 403 (this case), the tokenised one is 200 with no
    // Firebase app, no Authorization header and no account at all. The
    // token sits on `v2_avatars/{uid}`, which any signed-in — so any free
    // anonymous — account may read, so the link is obtainable and then
    // shareable.
    //
    // BOTH NIGHT SHIFTS FOUND THIS WITHIN AN HOUR OF EACH OTHER and fixed
    // different halves, which is why this comment is a merge of two.
    // `storage.rules` carries the argument and what changing the
    // mechanism would cost; `web/privacy.html` no longer promises the
    // floor this case cannot deliver, and `check:policy-claims` pins the
    // sentence that replaced it. The other option is priced rather than
    // dismissed: drawing faces through `getBytes`/`getBlob` into an object
    // URL restores the floor and costs the plain <img> plus the property
    // avatar.ts banks on — a user who never sets a photo never pays for
    // `firebase/storage`.
