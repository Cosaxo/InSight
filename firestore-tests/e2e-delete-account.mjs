...
// The voter samples (D385): the one derived public document family that
...
if (sampleAfter.get("rows")?.[uid] !== undefined)
  fail("the erased account's row survived in a world-readable voter sample (D385)");
...
for (const [path, label] of [
  ...
  [`v2_users/${uid}/patterns/state`, "the fit's per-person state and answer map (D383)"],
