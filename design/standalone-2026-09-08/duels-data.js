(function () {
  ...
  const GROUP_QS = [{
    ...
    options: ["Honesty", "Loyalty", "Chaos", "Calm"]
  }, {
    id: "gd3",
    prompt: "Best dinner together: cook, or book a table?",
    options: ["Cook together", "Book a table"]
  }, {
    id: "gu3",
    kind: "us",
    prompt: "When we disagree, we\u2026",
    options: ["Talk it out", "Vote", "Let it slide", "Loudest wins"]
  }, {
    id: "gp1",
    kind: "pick",
    ...
    options: ["So loud", "So close", "So weird", "So fun"]
  }, {
    id: "gd0",
    prompt: "A cabin with no wifi. How long do you last?",
    options: ["One night", "Two", "Longer", "Move me in"]
  }, {
    id: "gu5",
    kind: "us",
    prompt: "What are we most likely to be late for?",
    options: ["Nothing", "Everything", "Dinner", "The airport"]
  }, {
    id: "gp2",
    kind: "pick",
    ...
    id: "gu2",
    kind: "us",
    prompt: "Our default plan for a free evening?",
    options: ["Big dinner", "Out out", "Sofa + film", "Spontaneous"]
  }, {
    id: "gd4",
    prompt: "A surprise party for you \u2014 love it or dread it?",
    options: ["Love it", "Dread it"]
  }, {
    id: "gu6",
    kind: "us",
    prompt: "The thing we never say out loud?",
    options: ["I miss you", "You were right", "I need help", "We say everything"]
  }, {
    id: "gp3",
    kind: "pick",
    ...
    options: ["Same but older", "Scattered, still close", "Neighbours", "A yearly reunion"]
  }, {
    id: "gd6",
    prompt: "On the road trip, you are the\u2026",
    options: ["Driver", "DJ", "Navigator", "Snacks"]
  }, {
    id: "gu8",
    kind: "us",
    prompt: "Our group chat is mostly\u2026",
    options: ["Plans", "Memes", "Life updates", "Silence"]
  }, {
    id: "gp4",
    kind: "pick",
    ...
    options: ["Nothing", "Distance", "Money", "A secret"]
  }, {
    id: "gd7",
    prompt: "Group holiday: one house together, or rooms apart?",
    options: ["One house", "Rooms apart"]
  }, {
    id: "gp5",
    kind: "pick",
    ...
    options: ["Late night", "Long dinner", "Outdoors", "Doing nothing"]
  }, {
    id: "gp6",
    kind: "pick",
    ...
    options: ["Open door", "Slow to warm", "Full \u2014 sorry", "Depends who"]
  }];
  const GROUPS = [{
    id: "g1",
    name: "The Crew",
  ...
  function groupQ(gid, r) {
    if (isWorldRound(r)) return worldQ("g" + gid + ":" + r);
    const k = (r - 1) / 2;
    let q = GROUP_QS[(gBase(gid) + k) % GROUP_QS.length];
    if (q.kind === "pick") {
      const act = activeMembers(gid);
      if (act.length < 2) {
        const idx = (gBase(gid) + k) % GROUP_QS.length;
        for (let i = 1; i < GROUP_QS.length; i++) {
          const cand = GROUP_QS[(idx + i) % GROUP_QS.length];
          if (cand.kind !== "pick") {
            q = cand;
            break;
          }
  ...
  function memberAnswer(gid, r, pid) {
    ...
    const maj = Math.floor(h01("gm" + gid + q.id + r) * n);
    if (h01("gp" + gid + q.id + r + pid) < 0.52) return maj;
    return Math.floor(h01("ga" + gid + q.id + r + pid) * n);
  }
  const gState = (gid, r) => (S.groups[gid] || {})[r] || null;
  ...
  function myGroupRound(gid, r) {
    ...
    if (r <= gHist(gid)) {
      const n = q.options.length;
      const a = Math.floor(h01("gs" + gid + q.id + r) * n);
      const maj = Math.floor(h01("gm" + gid + q.id + r) * n);
      const g = h01("gc" + gid + q.id + r) < 0.62 ? maj : (maj + 1 + Math.floor(h01("gcw" + gid + r) * (n - 1))) % n;
      return {
        a,
        g,
        late: false
    ...
    return {
      a: s ? s.a : null,
      g: s ? s.g != null ? s.g : null : null,
      late: !!(s && s.late)
    };
  }
  const myGroup = (gid, idx) => {
    const r = idx === 0 ? groupOpen(gid) || groupMyTop(gid) + 1 : groupRevealedCount(gid) - idx + 1;
    const m = myGroupRound(gid, r);
    return m.a;
  };
  ...
  const needsCall = (gid, r) => {
    const m = myGroupRound(gid, r);
    return m.a != null && m.g == null && !m.late && r > gHist(gid);
  };
  function groupDone(gid) {
    if (gid) {
      const r = groupOpen(gid);
      return r == null && !needsCall(gid, groupMyTop(gid));
    }
  ...
  function groupPicksRound(gid, r) {
    ...
    return {
      ...
      late: my.late,
      myCall: my.g,
      callRight: my.g != null && my.g === majority,
      counts,
      majority,
      played,
  ...
  const groupPicks = (gid, idx) => groupPicksRound(gid, idx === 0 ? groupOpen(gid) || groupMyTop(gid) : groupRevealedCount(gid) - idx + 1);
  function groupDays(gid) {
    const n = groupRevealedCount(gid);
    const open = groupOpen(gid) || groupMyTop(gid) + 1;
    return [{
      ...groupQ(gid, open),
      idx: 0,
      round: open,
  ...
  function groupAlignment() {
    ...
    groupDefs().forEach(G => {
      ...
      for (let r = 1; r <= n; r++) {
        ...
        if (g.mine == null || g.late) continue;
        total++;
        if (g.mine === g.majority) withMaj++;
      }
    });
    return {
      withMaj,
  ...
  function groupPortrait(gid) {
    ...
    for (let r = 1; r <= n; r++) {
      ...
      if (g.q.kind === "pick") {
        ...
        verdicts.push({
          prompt: g.q.prompt,
          label,
          who: label === "You" ? null : ms.find(p => p.name.split(" ")[0] === label) || null
        });
      }
    }
    ...
    return {
      days,
      rounds: days,
      togetherness: Math.round(togeth / Math.max(days, 1) * 100),
      meWithMaj,
  ...
  function groupView(gid) {
    ...
    const callDue = needsCall(gid, myTop) ? myTop : null;
    return {
      hist,
      revealed,
      open,
  ...
  const DUO_QS = [{
    ...
    options: ["The steady one", "The instigator", "The listener", "The truth-teller"]
  }, {
    d: "day",
  ...
  function duoQ(pid, r) {
    if (isWorldRound(r)) return worldQ("d" + pid + ":" + r);
    const depth = (r - 1) / 2;
  ...
  const dState = (pid, r) => (S.duo[pid] || {})[r] || null;
  ...
  function duoRound(pid, r) {
    ...
    const s = dState(pid, r) || {};
    const myAns = hist ? Math.floor(h01("da" + key) * n) : s.a != null ? s.a : null;
  ...
  function roleVotes(gid) {
    ...
    const all = [];
    SCENARIOS.forEach(sc => sc.roles.forEach(r => all.push({
      ...r,
      scen: sc,
      key: sc.id + ":" + r.id
    })));
