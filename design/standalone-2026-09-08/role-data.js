(function () {
  ...
  const RULE_ADJ = {
    duo: {
      read: ["off their read", "sharp on them"],
      seen: ["hard to call", "easy to call"],
      like: ["unalike", "alike"],
      steady: ["streaky", "steady"]
    },
    group: {
      own: ["along with it", "own-way"],
      pull: ["on the edge", "in the middle"],
      cast: ["unnamed", "first-picked"],
      settle: ["swinging", "settled"]
    }
  };
  const DIM_WORD = {
    duo: {
      read: ["less sharp on them", "sharper on them"],
      seen: ["harder to call", "easier to call"],
      like: ["less alike", "more alike"],
      steady: ["streakier", "steadier"]
    },
    group: {
      own: ["more along with it", "more own-way"],
      pull: ["further out", "closer to the middle"],
      cast: ["less named", "more often named"],
      settle: ["more swinging", "more settled"]
    }
  };
  const DUO_TYPES = [{
    name: "The Mind Reader",
    share: 7,
    line: "Calls their answer before they do.",
    sig: {
      read: 92,
      seen: 52,
      like: 55,
    ...
    name: "The Open Book",
    share: 12,
    line: "Easy to call, and fine with it.",
    sig: {
      read: 52,
      seen: 92,
      like: 58,
    ...
    name: "The Two-Way",
    share: 6,
    line: "You read each other equally well.",
    sig: {
      read: 86,
      seen: 86,
      like: 66,
    ...
    name: "The Stranger",
    share: 9,
    line: "Two people still guessing.",
    sig: {
      read: 18,
      seen: 20,
      like: 42,
    ...
    name: "The Twin",
    share: 13,
    line: "Same answer before either of you guesses.",
    sig: {
      read: 64,
      seen: 64,
      like: 94,
    ...
    name: "The Wildcard",
    share: 10,
    line: "Right, wrong, right \u2014 no pattern to hold.",
    sig: {
      read: 55,
      seen: 48,
      like: 50,
    ...
    name: "The Opposite",
    share: 11,
    line: "Never the same answer \u2014 you read each other anyway.",
    sig: {
      read: 72,
      seen: 68,
      like: 8,
    ...
    name: "The Watcher",
    share: 9,
    line: "Reads more than gets read.",
    sig: {
      read: 82,
      seen: 24,
      like: 50,
    ...
    share: 15,
    line: "Same call, week after week.",
    sig: {
      read: 60,
      seen: 60,
      like: 55,
  ...
  const GROUP_TYPES = [{
    name: "The Anchor",
    share: 13,
    line: "Where the group lands, you already were.",
    sig: {
      own: 12,
      pull: 84,
      cast: 68,
    ...
    name: "The Contrarian",
    share: 9,
    line: "The one vote against, most weeks.",
    sig: {
      own: 92,
      pull: 28,
      cast: 42,
    ...
    name: "The Bellwether",
    share: 12,
    line: "Vote with you and you vote with everyone.",
    sig: {
      own: 20,
      pull: 92,
      cast: 48,
    ...
    name: "The Wildcard",
    share: 11,
    line: "In with them, then out, no rhythm.",
    sig: {
      own: 58,
      pull: 44,
      cast: 38,
    ...
    name: "The First Pick",
    share: 8,
    line: "Whatever the scenario, your name comes up.",
    sig: {
      own: 46,
      pull: 54,
      cast: 94,
    ...
    name: "The Quiet Majority",
    share: 16,
    line: "With the group, never at the front.",
    sig: {
      own: 10,
      pull: 68,
      cast: 12,
    ...
    name: "The Outlier",
    share: 7,
    line: "Your own answer, every time.",
    sig: {
      own: 88,
      pull: 10,
      cast: 22,
    ...
    name: "The Spark",
    share: 10,
    line: "Pulls the group off its default.",
    sig: {
      own: 72,
      pull: 56,
      cast: 78,
  ...
  function duoDims(pid) {
    ...
    const run = key => Array.from({
      length: p.played
    }, (_, i) => !!D.duoDay(pid, p.played - i)[key]);
    const sameRun = Array.from({
      length: p.played
    }, (_, i) => {
      const d = D.duoDay(pid, p.played - i);
      return d.myAns != null && d.myAns === d.theirAns;
    });
    ...
    return {
      ...
      dims: [{
        id: "read",
        label: CFG.duo.labels.read,
        value: rate(p.read.right, p.read.total),
        note: `right on ${p.read.right} of your ${p.read.total} guesses`
      }, {
        id: "seen",
        label: CFG.duo.labels.seen,
        value: rate(p.readBy.right, p.readBy.total),
        note: `they\u2019re right on ${p.readBy.right} of their ${p.readBy.total}`
      }, {
        id: "like",
        label: CFG.duo.labels.like,
        value: rate(nSame, p.played),
        note: `same answer on ${nSame} of ${p.played} rounds`
      }, {
        id: "steady",
        label: CFG.duo.labels.steady,
        value: steadiness(readRun),
        note: `your read flipped ${flips} time${flips === 1 ? "" : "s"} in ${p.played} rounds`
      }]
    };
  }
  function groupDims(gid) {
    ...
    const majRun = P.voteRounds.map(r => {
      const pk = D.groupPicksRound(gid, r);
    ...
    return {
      ...
      dims: [{
        id: "own",
        label: CFG.group.labels.own,
        value: clamp(100 - P.meWithMaj / P.days * 100),
        note: `named someone the room didn\u2019t on ${P.days - P.meWithMaj} of ${P.days} votes`
      }, {
        id: "pull",
        label: CFG.group.labels.pull,
        value: clamp(pull * 100),
        note: `others named who you named ${wm} of ${ms.length * P.days} times`
      }, {
        id: "cast",
        label: CFG.group.labels.cast,
        value: cast,
        note: castNote
      }, {
        id: "settle",
        label: CFG.group.labels.settle,
        value: steadiness(majRun),
        note: `switched sides ${mflips} time${mflips === 1 ? "" : "s"} in ${P.days} votes`
      }]
    };
  }
  ...
  function duoDimsTheirs(pid) {
    ...
    const p = D.partners().find(x => x.id === pid);
    if (!p || p.played < MIN_DUO) return null;
    const byRun = Array.from({
      length: p.played
    }, (_, i) => !!D.duoDay(pid, p.played - i).byRight);
    const sameRun = Array.from({
      length: p.played
    }, (_, i) => {
      const d = D.duoDay(pid, p.played - i);
  ...
  function groupDimsFor(gid, pid) {
    ...
    let withThem = 0;
    const majRun = [];
    P.voteRounds.forEach(r => {
      const g = D.groupPicksRound(gid, r);
      const row = g.rows.find(rw => rw.who.some(w => w.id === pid));
      if (!row) {
        majRun.push(true);
        return;
      }
      withThem += g.counts[row.oi] - 1;
    ...
    return {
      ...
      dims: [{
        id: "own",
        value: clamp(100 - P.withMaj[pid] / P.days * 100)
      }, {
        id: "pull",
        value: clamp(pull * 100)
      }, {
  ...
  function sync() {
    ...
    duos.forEach(({
      p,
      r
    }) => put("duo:" + p.id, "duo", r, "Role \xB7 1v1 with " + String(p.name).split(" ")[0], "With " + String(p.name).split(" ")[0]));
    groups.forEach(({
      g,
      r
