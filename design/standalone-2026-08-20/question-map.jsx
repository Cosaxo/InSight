...
(function () {
  ...
  function QuestionMap({ topic, onUse }) {
    ...
    const restOp = (i) => 0.55 + Math.min(1, hubs[i] / hubMax / 0.72) * 0.45;
              ...
              {PL.pts.map((p) => {
                ...
                return (
                  <g key={i} onClick={(e) => { e.stopPropagation(); pick(i); }} style={{ cursor: 'pointer' }}>
                    ...
                    {i === nxt && <text className="qm-nextlab" x={Math.max(30, Math.min(W - 30, p.x))} y={p.y > H - 26 ? p.y - r - 9 : p.y + r + 15} textAnchor="middle" fill="var(--ink)" style={{ paintOrder: 'stroke', stroke: 'var(--surface-2)', strokeWidth: 3.5 }}>answer next</text>}
            ...
            {topSay && (
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 9, marginTop: 14 }}>
                ...
                <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--ink-2)', lineHeight: 1.4, textWrap: 'pretty' }}>
                  of the <b style={{ color: 'var(--ink)', fontWeight: 700 }}>{Q.length} questions</b> here are lit by your answers {'\u00b7'} <b style={{ color: 'var(--ink)', fontWeight: 700 }}>{all.length} ties</b> hold between them.
                </span>
              </div>
            )}
