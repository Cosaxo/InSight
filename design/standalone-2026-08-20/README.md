...
extracted here in two passes: the People lens at D214, the Map and
Oracle at D215. The upload is ephemeral; these files are the surviving
...
| `patterns-tab.jsx` | The tab shell: three-lens ruler, one sub-row (topic chips · population chips · oracle progress) | **Ported at D215** into `src/v2/ui/PatternsTab.tsx`, **except the population chips** — see below |

**Still unported, deliberately:** the People lens's per-population views
(`PAT.pops()` — world · circle — and their `pt-pops` sub-row). A circle
cut of the people map needs the circle's uids joined against the placed
crowd and its own thin-state reading; that is a real design with no
decision behind it yet. Until then the People sub-row holds its height
empty, and the lens is world-wide.

Behaviours the ports refused, each recorded in the shipped files'
comments and in D214/D215: invented names for nameless accounts; a
...
state (a live answer cannot be unanswered); and the tap-anywhere
advance on a clickable `<div>` (the a11y ratchet's exact case — Next
does the job).
