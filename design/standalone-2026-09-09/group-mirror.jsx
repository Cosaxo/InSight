(function () {
  ...
  function GroupScoresCard({
    ...
    return React.createElement("div", {
      ...
      return React.createElement("div", {
        ...
        const hi = s.counts[3] + s.counts[4],
          lo = s.counts[0] + s.counts[1],
          mid = s.counts[2];
        ...
        }, "\xB7 ", s.score, " \xB7 round ", s.r)), React.createElement("div", {
          style: {
            marginTop: 4,
            fontFamily: "var(--sans)",
            fontSize: 12,
        ...
        }, [hi ? `${who(hi)} lean ${s.poles[1]}` : null, lo ? `${who(lo)} lean ${s.poles[0]}` : null, mid ? `${who(mid)} in between` : null].filter(Boolean).join(" \xB7 "), s.mine != null ? ` · you said ${D.stepLabel(s.poles, s.mine)}` : ""));
      })());
    })), React.createElement("div", {
      style: {
        display: "flex",
  ...
  function GroupPeopleCard({
    ...
    const seats = ms.filter(m => !m.pending).map(m => ({
      m,
      A: D.archetypeOf ? D.archetypeOf(g.id, m.id) : null
    })).filter(s => s.A);
    ...
    return React.createElement("div", {
      ...
      style: small
    }, "closer \u2192 higher affinity")), React.createElement("div", {
      style: {
        position: "relative",
        height: BH,
        marginTop: 6
      ...
      return React.createElement("div", {
        ...
        role: "button",
        tabIndex: 0,
        "aria-label": `${p.name} · ${Math.round(p.match)} affinity`,
        onKeyDown: e => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
        ...
        style: {
          ...
          display: "inline-flex",
          marginBottom: 1,
          boxShadow: isTwin ? "0 0 0 2px var(--accent)" : "0 0 0 2.5px var(--surface)"
        }
      }, React.createElement(window.GDAv, {
        p: p,
        size: 28,
      ...
      }, p.name.split(" ")[0]), React.createElement("span", {
        style: {
          fontFamily: "var(--sans)",
          fontSize: 10,
          fontWeight: 700,
    ...
    })), twinOn && React.createElement("div", {
      className: "legend",
      style: {
        justifyContent: "center",
        marginTop: 2
      ...
      className: "lg-dot"
    }), "casts the room like you")), anySeat && React.createElement("div", {
      style: {
        marginTop: 13,
        paddingTop: 13,
        borderTop: LINE,
      ...
      return React.createElement("div", {
        ...
        style: {
          ...
          color: named ? "var(--accent)" : "var(--ink-3)",
          overflow: "hidden",
          textOverflow: "ellipsis",
          whiteSpace: "nowrap"
        }
      }, named ? A.seat.line : "not named yet"), named && React.createElement("span", {
        style: {
          fontFamily: "var(--sans)",
          fontSize: 11,
          fontWeight: 600,
      ...
      }, A.shares[A.seat.id], " of ", A.total, " votes"));
    })), shared.length > 0 && React.createElement("div", {
      style: {
        marginTop: 13,
        paddingTop: 13,
        borderTop: LINE,
    ...
    }, shared.map(([t, n]) => React.createElement("span", {
      key: t,
      style: {
        fontFamily: "var(--sans)",
        fontSize: 12.5,
  ...
  function GroupsMirrorBody({
    ...
    if (!g) return null;
    const P = D.groupPortrait(g.id);
    const nCast = D.roleVotes(g.id).roles.length,
      nScores = D.groupScores(g.id).length;
    const castPct = Math.round(nCast / Math.max((D.ALL_ROLES || []).length, 1) * 100);
    const lenses = [{
      id: "votes",
      label: "Votes",
    ...
    return React.createElement("div", {
    ...
    }, nCast || nScores ? `${nCast} role${nCast === 1 ? "" : "s"} cast · ${nScores} score${nScores === 1 ? "" : "s"}` : "no rounds revealed yet"))), React.createElement(GroupPicker, {
      gs: gs,
      cur: g.id,
      onPick: setGid
      ...
      open: openId,
      onOpen: setLensOpen
    }), (!topLenses || openId === "__ov") && React.createElement(React.Fragment, null, (() => {
      const A = D.archetypeOf ? D.archetypeOf(g.id, "me") : null;
      return A && A.total >= 2 && A.seat ? React.createElement("div", {
        style: {
          margin: "12px 2px 0",
          display: "flex",
          alignItems: "baseline",
      ...
      }, "Here, you are ", React.createElement("span", {
        style: {
          color: "var(--accent)"
        }
      }, A.seat.line)), React.createElement("span", {
        style: {
          fontFamily: "var(--sans)",
          fontSize: 12,
          fontWeight: 500,
      ...
      }, A.shares[A.seat.id], " of ", A.total, " votes say so")) : null;
    })(), React.createElement(window.GroupRoleMap, {
      key: g.id,
      gid: g.id,
      gname: g.name
