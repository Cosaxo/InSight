...
function OracleLens({ onUse }) {
  ...
  return (
    <div className="or-lens">
      ...
      <div key={curQ.id + '-inst'} className={'or-inst' + (rec ? ' is-live' : '')} style={{ '--d': dsty['--d'] }} onClick={rec ? next : undefined}
      ...
      {why && !rc && (
        <div className="or-proof">
          ...
          {work && work.rows.length ? work.rows.map((r, k) => {
            const evq = qOf(r.id); if (!evq) return null;
            const sh = r.t.share[rec.pred];
            const wmax = work.rows[0].w || 1;
            return (
              <div className="or-ev" key={r.id} style={{ animationDelay: k * 80 + 'ms' }}>
                <span className="or-ev-q">You said <b>{evq.options[r.t.side].label}</b>{'\u2009\u2014\u2009'}{'\u201c' + evq.prompt + '\u201d'}</span>
                <span className="or-ev-row">
                  <span className="or-ev-bar"><i style={{ width: Math.round(sh * 100) + '%', background: evFill, opacity: 0.55 + 0.45 * Math.min(1, r.w / wmax) }}></i><em></em></span>
                  <span className="or-ev-word">{orWord(sh)} pick <b>{curQ.options[rec.pred].label}</b> {'\u00b7'} {r.t.n} counted</span>
                </span>
              </div>
            );
          }) : <span className="or-ev-none">Nothing in your answers pointed either way here {'\u2014'} it guessed at the coin, and the empty disc says so.</span>}
