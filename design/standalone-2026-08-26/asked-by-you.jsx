...
(function () {
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-end', gap: 3 }}>
          {cols.map((c, i) => (
            <div key={i} style={{
              ...
              height: c.t === 'bar' || c.t === 'thin' || c.t === 'after' ? c.h : c.t === 'gap' ? 5 : 0,
              background: c.t === 'bar' ? 'color-mix(in oklch, var(--c-city) 55%, var(--surface-2))' : c.t === 'after' ? 'color-mix(in oklch, var(--ink) 15%, var(--surface-2))' : 'transparent',
    ...
    return (
      <div className="card" style={{ marginTop: 12, padding: '13px 14px' }}>
        ...
        <div style={{ marginTop: 5, fontFamily: 'var(--sans)', fontSize: 11.5, fontWeight: 650, color: 'var(--ink-2)', fontVariantNumeric: 'tabular-nums' }}>
          <span style={{ fontWeight: 800, color: 'var(--ink)' }}>{fmt(total)}</span> of {fmt(cap)} answers bought · {pct}% — bills {P.fmt(p.rate || P.PRICE.perAnswer)} / answer, {p.lockedIdx || '×1.0'} locked at booking
        </div>
        ...
        <div style={{ marginTop: 5, fontFamily: 'var(--sans)', fontSize: 10.5, fontWeight: 600, color: 'var(--ink-3)', lineHeight: 1.45 }}>
          {p.closed
            ? (p.atClose === 'settle' ? 'settled at close — paid what arrived, never the cap' : 'met after a free extension — the deadline moved, the rate didn\u2019t')
            : 'the window is a deadline — short at close: ' + (p.atClose === 'settle' ? 'settle for what arrived' : 'extends free until met') + ' (picked at booking)'}
        </div>
  ...
  // Co-funded: the period cost splits evenly across seats (WF_PAID.SUB).
  // Illustrative, like WF_PAID's questions; a lapse keeps the series (law 08).
  const SUBS = [
    { id: 'sub1', metric: 'Do buses come when the app says they will?', cohort: 'Oslo', score: '4.0', since: '2 May', state: 'active', seed: 1.3, thin: 17, gap: 9, seats: 3 },
    { id: 'sub2', metric: 'How safe does the city feel after dark?', cohort: 'Oslo', score: '3.6', since: '12 Mar', state: 'lapsed', lapsedAt: '17 Aug', seed: 0.7, thin: 7, gap: 13, seatsLeft: 2 },
  ];
  function SubCard({ s }) {
    const P = window.WF_PAID;
    const lapsed = s.state === 'lapsed';
    const runsOn = lapsed && (s.seatsLeft || 0) > 0;
    const seats = s.seats || s.seatsLeft || 0;
    const cols = mkSeries(26, s.seed, s.thin, s.gap, lapsed ? 19 : null, runsOn);
    return (
      <div className="card" style={{ marginTop: 12, padding: '13px 14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
          <span style={K}>subscription · rates: city · {s.cohort}{seats ? ' · ' + seats + ' seats' : ''}</span>
          {lapsed ? <StateChip label={runsOn ? 'seat lapsed' : 'lapsed'} hollow /> : <StateChip label="active" acc="var(--c-city)" />}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--sans)', fontSize: 11, fontWeight: 600, color: 'var(--ink-3)' }}>{lapsed && !runsOn ? 'at the lapse' : 'this month'} · n 44–212 / day · since {s.since}</span>
        </div>
        <Series cols={cols} cut={lapsed ? 19 : null} cutLabel={lapsed ? (runsOn ? 'your seat lapsed ' + s.lapsedAt : 'lapsed ' + s.lapsedAt) : null} />
        <SeriesBasis />
        {lapsed ? (
          <div style={{ marginTop: 10, borderTop: '1px solid color-mix(in oklch, var(--rule) 62%, transparent)', paddingTop: 10 }}>
            <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 650, color: 'var(--ink-2)', lineHeight: 1.45 }}>{runsOn ? 'Your seat lapsed — ' + s.seatsLeft + ' others still fund it, so the series runs on (dimmed since your lapse). Rejoin and it stays one series, not a fork.' : 'Yours was the last seat — paused, history kept. Re-subscribe and the series continues, not forks.'}</div>
            <div style={{ marginTop: 9, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: 'var(--sans)', fontSize: 10.5, fontWeight: 600, color: 'var(--ink-3)' }}>{runsOn ? 'same qid · rejoin at ' + P.fmt(P.SUB.seat('city', s.seatsLeft + 1)) + ' — the split recomputes' : 'same qid · first seat pays ' + P.fmt(P.SUB.perPeriod('city')) + ' — each join splits it'}</span>
            </div>
          </div>
        ) : (
          <div style={{ marginTop: 10, borderTop: '1px solid color-mix(in oklch, var(--rule) 62%, transparent)' }}>
            ...
            <div style={{ fontFamily: 'var(--sans)', fontSize: 10.5, fontWeight: 600, color: 'var(--ink-3)' }}>{'billed monthly · your seat ' + P.fmt(P.SUB.seat('city', s.seats)) + ' of ' + P.fmt(P.SUB.perPeriod('city')) + ', split ' + s.seats + ' ways · next report 1 Sep'}</div>
          </div>
        )}
      </div>
    );
