...
function mtClusterLayout(nodes, cats) {
  ...
  const ringSkip = new Set(); // bullseye members keep their exact radius — no typ push
  const cl = cats.map((cat, i) => {
    const h2 = mtHash(cat.id + 'x');
    const rot = h2 * Math.PI * 2;
    // a ring branch is a bullseye: cuts sit on rays around the hub, and every
    // dot's distance from the hub is its own accuracy (typ) — closer = sharper
    if (cat.ring) {
      const kids = byParent[cat.id] || [];
      const local = {};
      const R0 = 58, SPAN = 118; // compact \u2014 sits like the other branch clouds
      const rr = (t) => R0 + (1 - Math.max(0.2, Math.min(0.94, t == null ? 0.5 : t))) * SPAN;
      kids.forEach((k, j) => {
        const ka = rot + (j * 2 * Math.PI) / (kids.length || 1);
        const kr = rr(typOf(k)); // the cut hub rides its own average
        local[k.id] = { x: Math.cos(ka) * kr, y: Math.sin(ka) * kr };
        ringSkip.add(k.id);
        const gks = byParent[k.id] || [];
        const step = ((2 * Math.PI) / (kids.length || 1)) / (gks.length + 0.6);
        const off = gks.length % 2 ? 0.5 : 0; // odd fans nudge off the hub's ray
        gks.forEach((g, m) => {
          const ga = ka + (m - (gks.length - 1) / 2 + off) * step;
          const gr = rr(g.typ);
          local[g.id] = { x: Math.cos(ga) * gr, y: Math.sin(ga) * gr };
          ringSkip.add(g.id);
        });
      });
      return { id: cat.id, i, local, radius: R0 + SPAN + 26, x: 0, y: 0, ringR: [rr(0.8), rr(0.6), rr(0.4)] };
    }
  ...
  for (let pass = 0; pass < 220; pass++) {
    ...
    cl.forEach((c) => {
      ...
      // a bullseye's inner dots must clear the anchor ring on their own
      const need = (c.ringR ? 205 : CLEAR) + c.radius;
      if (d < need) d = need;
  ...
  cl.forEach((c) => {
    ...
    for (const id in c.local) {
      ...
      const n = nById[id];
      if (n && n.typ != null && !ringSkip.has(id)) {
  ...
  // bullseye geometry for the renderer — hairline accuracy rings
  const rings = cl.filter((c) => c.ringR).map((c) => ({ id: c.id, x: c.x, y: c.y * VS, radii: c.ringR }));
  return { pos, fields, rings };
}
