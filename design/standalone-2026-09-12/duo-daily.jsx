(function () {
  ...
  function DuoCard({
    ...
    const waitBlock = () => {
      ...
      }, name, "’", "s answer", rd.noGuess ? "" : React.createElement(React.Fragment, null, " ", "·", " ", React.createElement("span", {
        style: {
          color: "var(--ink)",
          fontWeight: 800
        }
    ...
    const sealedBlock = () => React.createElement(window.SealedList, {
      ...
      items: v.sealed.map(r => {
        ...
        return {
          n: r,
          prompt: rd.q.prompt,
          sub: (rd.q.world ? "World · " : "") + "you said " + rd.q.options[rd.myAns] + (rd.myGuess != null ? " · called " + guessOpts(rd)[rd.myGuess] : "")
        };
      })
    });
    let body;
  ...
  function NewDuoSheet({
    ...
    if (!avail.length) {
      return React.createElement("div", {
      ...
      }, "Everyone in your circle already has a 1v1 with you.", React.createElement("div", {
        style: {
          marginTop: 4
        }
        ...
        onClick: () => {
          close();
          window.openOverlay && window.openOverlay("friends");
        },
        style: {
          alignSelf: "center",
          marginTop: 4,
    ...
    return React.createElement(React.Fragment, null, avail.map(p => React.createElement("button", {
      key: p.id,
      className: "press",
      onClick: () => {
        D.startDuo(p.id);
    ...
    }, "Invite"))), React.createElement("button", {
      key: "find",
      className: "press",
      onClick: () => {
        close();
