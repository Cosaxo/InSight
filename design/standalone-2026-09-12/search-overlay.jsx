const {
  useState: useSrchState,
  useEffect: useSrchEffect,
  useMemo: useSrchMemo,
  useRef: useSrchRef
} = React;
...
function srchVotes() {
  try {
    const v = JSON.parse(localStorage.getItem(SRCH_VOTES_LS) || "{}");
...
function srchQScore(q, query, topicLabel) {
  ...
  (q.options || []).map(o => o.label).concat(q.items || []).forEach(l => {
    const s = srchWhere(l, query);
    if (s >= 0) best = Math.max(best, 6 + s);
  });
...
function SrchMark({
  ...
  if (i < 0) return text;
  return React.createElement("span", null, text.slice(0, i), React.createElement("mark", null, text.slice(i, i + q.length)), text.slice(i + q.length));
}
function SrchHit({
  glyph,
  title,
...
function SrchQRow({
  ...
  return React.createElement("button", {
    ...
    style: {
      display: "block",
      lineHeight: 1.3,
      fontWeight: answered ? 600 : 700,
      color: answered ? "var(--ink-2)" : "var(--ink)"
    }
  }, React.createElement(SrchMark, {
    text: q.prompt,
    q: query
  ...
  }, pick)), answered ? React.createElement(SrchShare, {
    q: q,
    votes: votes,
    color: color
  }) : React.createElement("span", {
...
function SrchTopicRow({
  ...
  }, item.n, " question", item.n === 1 ? "" : "s")), React.createElement("button", {
    className: "press",
    onClick: onToggle,
    style: {
      flexShrink: 0,
      border: on ? "0.5px solid var(--rule)" : "none",
      borderRadius: 999,
      padding: "7px 14px",
      cursor: "pointer",
      fontFamily: "var(--sans)",
      ...
      color: on ? "var(--ink-3)" : "var(--surface)",
...
function SearchOverlay({
  ...
  const labelOf = qq => {
    if (qq.scene && SC) {
      const g = SC.defs().find(x => x.id === qq.scene);
      if (g) return {
        label: g.name,
        color: SC.colorOf ? SC.colorOf(g.id) : "var(--ink-3)"
      };
    }
    if (qq.sub && ST) {
      const s = ST.get(qq.sub);
      if (s) return {
        label: s.label,
        color: window.WPAL.c((TOPIC[s.parent] || {}).color) || "var(--ink-3)"
      };
    }
    const t0 = TOPIC[qq.cat];
    return t0 ? {
      label: t0.label,
      color: window.WPAL.c(t0.color)
    } : {
      label: qq.cat,
      color: "var(--ink-3)"
    };
  ...
  const questions = useSrchMemo(() => {
    ...
    if (!query) {
      ...
      for (let i = 0; out.length < 5 && order.some(k => i < by[k].length); i++) {
        order.forEach(k => {
          if (out.length < 5 && i < by[k].length) out.push(by[k][i]);
        });
      }
      return out;
    }
    return pool.map(x => ({
      x,
      s: srchQScore(x, query, labelOf(x).label)
    })).filter(r => r.s >= 0).sort((a, b) => b.s - a.s || (srchAnswered(a.x, votes) ? 1 : 0) - (srchAnswered(b.x, votes) ? 1 : 0) || srchQVotes(b.x) - srchQVotes(a.x)).slice(0, 12).map(r => r.x);
  }, [query, votes]);
  const topics = useSrchMemo(() => {
    ...
    if (ST) ST.all().forEach(s => {
      ...
      out.push({
        id: s.id,
        label: s.label,
        color: (TOPIC[s.parent] || {}).color || "var(--ink-3)",
        n: ST.count(s.id),
        on: ST.has(s.id),
        toggle: () => ST.toggle(s.id)
      });
    });
    if (SC) SC.defs().forEach(g => {
      ...
      out.push({
        ...
        n,
        on: SC.has(g.id),
        toggle: () => SC.toggle(g.id)
      });
    });
    const seen = {};
    return out.sort((a, b) => b.n - a.n).filter(t => {
  ...
  const dailies = useSrchMemo(() => {
    ...
    if (!query || !DQ) return [];
    return DQ.questions.map(x => ({
      x,
      s: srchQScore({
        prompt: x.prompt,
        options: (x.options || []).map(l => ({
          label: l
        }))
      }, query, [x.tag].concat(x.cat || []).filter(Boolean).join(" "))
    })).filter(r => r.s >= 0).sort((a, b) => b.s - a.s).slice(0, 8).map(r => {
      const mine = DQ.myAnswer(r.x);
      return {
        id: r.x.id,
        prompt: r.x.prompt,
        said: mine != null && r.x.options ? r.x.options[mine] : null
      };
    });
  }, [query]);
  ...
  const open = qq => {
    ...
    requestAnimationFrame(() => {
      const sc = scRef.current,
        row = sc && sc.querySelector("[data-openq]");
  ...
  const qRow = qq => {
    ...
    if (isOpen) {
      return React.createElement("div", {
        ...
        style: {
          position: "relative",
          border: "1px solid color-mix(in oklch, var(--accent) 32%, var(--rule))",
          borderRadius: 18,
      ...
      }, "✕"), React.createElement(window.WorldFeed, {
        focus: [qq],
        cats: {},
        onToggle: () => {},
        beats: false,
    ...
    return React.createElement(SrchQRow, {
      key: qq.id,
      q: qq,
      query: query,
      votes: votes,
  ...
  return React.createElement("div", {
    ...
    placeholder: "Questions, topics, people…",
    autoComplete: "off",
    autoCorrect: "off",
    autoCapitalize: "none",
    spellCheck: false,
  ...
  }, "Nothing for “", q, "” — try a topic, a name, or a few words of a question"), !!questions.length && React.createElement("div", {
    className: "search-group"
  }, query ? "Questions" : "Open questions"), questions.map(qRow), !!dailies.length && React.createElement("div", {
    className: "search-group"
  }, "Daily archive"), dailies.map(d => React.createElement("button", {
    key: d.id,
    className: "search-hit",
    style: {
      alignItems: "flex-start",
    ...
    onClick: () => go(() => window.goTab && window.goTab("track"))
  }, React.createElement("span", {
    "aria-hidden": "true",
    style: {
      width: 7,
    ...
    text: d.prompt,
    q: query
  })), d.said && React.createElement("span", {
    className: "hit-s",
    style: {
      display: "block"
    }
  }, "you said ", d.said)), React.createElement("span", {
    style: {
      color: "var(--ink-3)",
      fontSize: 13
    }
  }, "→"))), !!topics.length && React.createElement("div", {
    className: "search-group"
  }, "Topics"), topics.map(t => React.createElement(SrchTopicRow, {
    key: t.id,
    item: t,
    query: query,
    onToggle: () => {
  ...
  }, query ? "People" : "Friends", !query && React.createElement("button", {
    className: "press",
    onClick: () => go(() => window.openOverlay && window.openOverlay("friends")),
    style: {
      padding: 0,
      background: "none",
      border: "none",
  ...
  }, "Manage →")), people.map(p => React.createElement(SrchHit, {
    key: p.id,
    glyph: p.anon ? React.createElement(AnonAv, {
      hue: p.hue,
      size: 32
    ...
    title: anonName(p),
    sub: [p.role || p.rel, p.since ? "since " + p.since : null, p.match != null ? Math.round(p.match) + "% match" : null].filter(Boolean).join(" · "),
    q: query,
