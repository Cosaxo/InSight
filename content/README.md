...
| `topic-proposals.json` | The taxonomy ledger (D421) — questions a lane met that fit no existing category, parked with their run dates rather than dropped, plus the `created` rows. Read by `npm run topic:budget`, which rules on whether a gap has become a category (evidence · breadth debt · capacity · settling), and validated by `check:taxonomy`. **Ships empty, and that is its correct state**: proposals accumulate from runs, they are not seeded. A parked question reaches no bank until the category is created and it is written into one |
