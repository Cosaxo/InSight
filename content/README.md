...
| `topic-proposals.json` | The taxonomy ledger (D424, D425) — questions a lane met that fit nothing below their nearest parent, parked with their run dates rather than dropped, plus `retag` (existing questions a proposed leaf claims) and the `created` rows. Read by `npm run topic:budget`, which rules per proposal — a **leaf** (feed subtopic, learn field: evidence · parent levelled · one leaf per parent at a time) or a **top**, which must land in a hub that already exists on the You map (`group`; a hub that does not exist is the owner's call) — and validated by `check:taxonomy`. **Ships empty, and that is its correct state**: proposals accumulate from runs, they are not seeded. A parked question reaches no bank until its room exists and it is written into one |
