...
- **Convert on touch, and transpose the meter before you plan.** This
  ...
  (D39's follow-ups, then D104) — both times because rule 4 reports per
...
- **A conversion removes the load-order condition, never the data one.**
  ...
  default (D104 found six of the first and two of the second). The inner
...
- **Expect a conversion to RAISE the suppression count before it lowers
  ...
  hiding `react-hooks` findings, not just costing coupling (D104, verified
