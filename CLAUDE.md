...
- **Convert on touch, and transpose the meter before you plan.** This
  ...
  (D39's follow-ups, then D106) — both times because rule 4 reports per
...
- **A conversion removes the load-order condition, never the data one.**
  ...
  default (D106 found six of the first and two of the second). The inner
...
- **Expect a conversion to RAISE the suppression count before it lowers
  ...
  hiding `react-hooks` findings, not just costing coupling (D106, verified
