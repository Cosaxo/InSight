...
function healthy(): NightlyRunners & { ran: string[] } {
  ...
  return {
    ...
    patterns: async () => {
      ran.push("patterns");
      return {
        days: 1, folded: 5, compacted: 5, samples: 2, citySamples: 1, users: 3, questions: 10, bits: 0.9, skill: 0.1,
    ...
    log: async () => { ran.push("log"); return { day: "2026-09-05", skipped: false, entries: 7, missing: 0, appended: 0, erasures: 0, erased: 0 }; },
...
describe("runNightlyPass", () => {
  it("runs the five in order and beats every heartbeat on a clean night", async () => {
    ...
    expect(r.ran).toEqual(["digest", "patterns", "taste", "velocity", "answerMaps", "log", "attention", "rollup"]);
  ...
  it("a night with nothing owed still beats for the fit (days counts), and a fold with no days stays quiet for taste", async () => {
    const r = healthy();
    r.patterns = async () => ({
      days: 1, folded: 0, compacted: 0, samples: 0, citySamples: 0, users: 0, questions: 10, bits: 0, skill: 0,
