...
/** Shard deletes the vote arm's batch issues (D388) — recorded apart from
...
const fakeDb = {
  ...
  // batch since D388; a set lands like a direct one, a delete is recorded.
  batch() {
...
describe("runRebuild's write-side refusals", () => {
  ...
  it("…and allowEmpty is the deliberate way through, all the way to the write", async () => {
    ...
    // …and the tail goes with it (D388): an empty fold has no shards, so
