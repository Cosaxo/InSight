...
// since v2 each slot draws its family from a same-weight band (D54) — a
...
// Practice attempts stay on-device; verified attempts (D55) are seeded and
...
const FAMILIES: Record<string, Family> = {
  ...
  // ── families added with the banded template (D54) ──
...
// v2 (D54): each slot draws its family from a same-weight band, without
