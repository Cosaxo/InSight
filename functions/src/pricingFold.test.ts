...
describe("foldPricing — the index is crowding beyond the free places, with no ceiling (D373, D377)", () => {
  ...
  it("averages over the fortnight — a campaign covering half of it is half a place", () => {
    ...
    // `crowd` is optional on PricingCohort (it arrived at D373), so it is
    // asserted present before it is sliced rather than assumed — the fold
    // publishing no crowd row at all is exactly what this case would
    // otherwise read as an empty one.
    expect(half.cohorts.city.crowd, 'the fold published no crowd row').toBeTruthy();
