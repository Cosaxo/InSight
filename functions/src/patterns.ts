...
// online engine) and patternsAls.ts (the candidate, D383), both pure; this
...
//
// TWO ENGINES, ONE DOCUMENT (D383). The shipped online fit measured as
// never leaving its hash seeds under the app's create-only regime (D382,
...
/** The CANDIDATE engine's corpus (D383): every option-shaped core item,
 ...
 * WHOLE (D383) — a field-by-field projection is how the retry stamp
...
export async function runPatternsFit(
  ...
  const pub: PatternsPublication = {
    ...
    // whether the vectors have learned anything at all (D382), over the
...
export function firestorePatternsStore(db: Firestore): PatternsStore {
  ...
  return {
    ...
    async getModel() {
      ...
      // and normalising the pre-D383 shape is the projection that cannot
      // drop anything.
        ...
        snaps.forEach((snap, j) => {
          if (snap.exists) {
            out.set(chunk[j], {
              ...
              // The answer map, both ways too (D383): the compaction merges
