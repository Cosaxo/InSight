...
export const deleteAccount = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      //
      // `worldSamples` and not `world`: D462's people-map scrub below took
      // that name for its own query, and this is the same block scope.
      let worldSamples = db.collection("v2_patterns")
        .where(FieldPath.documentId(), ">=", "sample-")
        .where(FieldPath.documentId(), "<", "sample.")
        .orderBy(FieldPath.documentId())
        .limit(WORLD_SAMPLE_PAGE);
      for (;;) {
        const page = await worldSamples.get();
        if (page.empty) break;
        await scrub(page.docs);
        if (page.size < WORLD_SAMPLE_PAGE) break;
        worldSamples = worldSamples.startAfter(page.docs[page.size - 1]);
      }
