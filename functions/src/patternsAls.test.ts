// The candidate engine's contract (D395), pinned the way patternsFit's is:
//
...
describe("the streamed solve (DATA-EFFICIENCY-RUNBOOK 4.3)", () => {
  const scanOf = (crowd: readonly { uid: string; a: AnswerMap }[]): PeopleScan => async (each) => {
    for (const p of crowd) each(p.uid, { a: p.a });
  };
  ...
  it("scans once for the statistics and once per sweep, and holds no person between", async () => {
    ...
    let scans = 0;
    const scan: PeopleScan = async (each) => { scans += 1; for (const p of crowd) each(p.uid, { a: p.a }); };
...
describe("anchors as items (D455)", () => {
...
describe("catalogue picks as items (D456)", () => {
...
describe("the link's slope (D457)", () => {
