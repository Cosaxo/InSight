// rank.ts — the nightly published serving order (D302; the spine D303's
...
//
// WHY A PUBLISHED ORDER EXISTS AT ALL. D302 pages the bank: a device
...
// read) per device, which is the whole cost argument of D302.
//
...
// already publishes, and it is personal to nobody — D163/D303's line
// holds because this fold never reads a uid. The sink is D302 phase 4's
...
//
// SURFACES: feed and learn — the two D302 unbounds. The daily is
...
/** The surfaces the order covers — the two D302 pages. */
export const RANK_SURFACES = ["feed", "learn"] as const;
export type RankSurface = (typeof RANK_SURFACES)[number];

/** The landslide sink (D302 phase 4's first signal): at this many answers
...
const BASIS = "volume desc, landslides sink (D302); ties by seq";
