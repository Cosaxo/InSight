...
/**
 * The daily's shape document (D385) — NOT an order, and the distinction
...
export interface DailyShapeDoc {
  ...
  maxSeq: number;
  /**
   * The Scores lens's pool, per place scope (D386) — question ids, not
...
export async function runBankRank(
  ...
  // The daily's shape rides the same nightly write (D385). It is not an
