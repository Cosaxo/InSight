...
| Database | `theory/database/` | Infrastructure theory, deliberately NOT an axiom (the owner, 2026-08-26): the perfect, most efficient and most useful database for the axioms and their connections — the join as the unit of design, custody as layout, storage shapes per reader, schema evolution. Redesigns at any size are licensed (the owner, 2026-08-28): nothing implements this design yet — no system, no data, no reader, no migration cost — so a run may re-found large parts of the theory in one firing rather than only evolving it node by node; §3's "one bounded improvement" bounds the run, not the size of the redesign that is the improvement, and the schema-evolution discipline this lane designs is subject matter for the designed system, never caution about churning the design itself. The ratchet and the ladder hold unchanged: a redesign lands as visible motion with its LOG row naming what was torn up and why, and a re-founded claim re-enters at the status its evidence supports rather than inheriting the old one (§4 — carrying `cited` onto a reshaped claim is the sin). Reads the others; does not write them |
