...
**Status: mixed — §2.1 and §2.2 are BUILT (D312, 2026-08-26); §4 is a
...
answers?", and built the same day on the owner's direction — D312 is the
...
| The agg-events ledger | **TTL-bounded** | 90 days (`LEDGER_RETENTION_DAYS`), and the nightly pass pages it in 5,000-doc `select()` pages (D399 — one read serving the digest, the fit, the taste fold, the heal and, since DATA-EFFICIENCY-RUNBOOK 4.4, the velocity scan, whose own read is the partial-day tail, per D64) |
...
### 2.1 · Phase 1 — instrument the swallow (S) · **BUILT (D312)**
...
### 2.2 · Phase 2 — one IndexedDB store for the hand caches (M) · **BUILT (D312)**

> `data/cacheStore.ts`, same day — the trigger collapsed to "the owner
> said build". As built with three recorded deviations, D312: the feed
...
D350's amendment, 2026-09-01: `INSTALL_WARN`, a warning on what a fresh
...
1. ~~**Instrument the quota swallow (§2.1).**~~ **Done — D312.** The
