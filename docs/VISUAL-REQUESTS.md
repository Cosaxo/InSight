...
- **title · asked by** — *Build the missing cell* · the owner, 2026-09-11,
  choosing the Open Matrices Item Bank over three alternatives (D472),
...
- **vocabulary** — the overlay's own tiles and accent (`LOGIC_COL`), the
  ...
  read** (D472's amendment): this paragraph said OMIB items are pictures
...
- **why** — D472: the items are calibrated before launch, which is what
...
- **status** — `built` 2026-09-12 (D474), the same day it was designed. Numbered 13 until the 2026-09-12 merge into `main`, where the Map ring's 13 (D464) had landed first — the D299 rule, one register over. `src/v2/spec/logic-test.jsx` is the screen: the board composing the goal cell live from `src/v2/data/omib-shapes.ts`, the palette laid out column = family / row = member with tap-to-toggle, Clear appearing once something is placed, Done committing (dormant while empty, pressable throughout — an item a taker cannot solve is skipped by committing the empty cell rather than by waiting out the clock), the current segment draining over its 90 s with the numeral in the final 20 s, the sitting's remainder in the header, and a time-out that commits the cell as it stands. One attempt on the one screen since D476 — Start IS the verified attempt, `logicStartV2`/`logicSubmitV2` (or `logicNextV2` item by item), the cells the only payload; the practice mode D473 put on the same screen lasted a day (the owner: *"this should be similar to an iq test"*). The two build notes were honoured — the shapes are the bank's, in the bank's order, arrows out (`omib-shapes.test.ts`). Fourteen cases in `logic-overlay.test.jsx`; the smoke mount renders the example. Since D475 the same screen also walks an ADAPTIVE form — one item arriving per Done, the strip drawn at the form's full length from the start, a lost pick held under Retry — behind the server's selection, which is stratified until the §6 report speaks. The graded haptics are wired through `spec/haptics.js` — place `tick`, Done `tap`, remove and the clock's commit silent — felt on Android, silent on iOS until a Capacitor bridge exists (D474 records the correction: this line first said the overlay had no haptic channel). What was designed and NOT built: the drag alternative, which the design itself put second.
