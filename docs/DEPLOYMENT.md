...
| `ANTHROPIC_API_KEY` *(secret)* | `functions/src/paid.ts` | The Claude API key the automated paid-question review calls (`claude-opus-5` against `REVIEW_GUIDELINES`). Unset ⇒ the review is DEFERRED, not decided: `runReviewVerdict` throws `ReviewDeferred`, the booking stays in `review` with no attempt counted, and the review Routine (`scripts/paid-review.mjs`, docs/ROUTINES.md) is what settles it — a booking sitting in `review` IS that queue. The deploy warning still names the missing key. This row described the pre-D456 behaviour — "reviews decide on the deterministic gates alone, logged as `paid_review_gates_only` — fail-open ONLY past the gates" — which is the line the code itself calls the most dangerous it has carried, and which now happens in the EMULATOR only (paid.ts guards it on `FUNCTIONS_EMULATOR`, and `scripts/observe.mjs` already records that the metric can no longer appear in a deployed runtime). |
