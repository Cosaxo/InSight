...
- **App icon — done, but it is a first pass.** Both shells shipped the
  ...
  - Still owed for the listings, and *not* generated by that script:
    store screenshots, the Play feature graphic (1024×500), and the
    marketing copy. Those are per-store and want real device captures.
