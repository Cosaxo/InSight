...
- **Legal pages — three values owed, deliberately.** `public/terms.html`
  ...
  nobody reads. That matters more than it looks — `public/privacy.html`
  ...
  the check.
- **No hosting target, so the policy has no URL.** Both store listings
  require a *reachable* privacy-policy link, and `firebase.json` has no
  `hosting` block — `public/terms.html` and `public/privacy.html` ship
  inside the app bundle and are reachable from the privacy panel, but they
  are not on the web. One hosting target fixes this and, per D3's
  amendment, is the same target that would unblock invite deep links.
