...
- [ ] `[claude-3]` Rounds step 4+5 — the late answer (`late: true` required by the rules when the reveal exists, no `guessIdx` with it, a server-only append, excluded from the dims, the ledger and `duel-{qid}`) and the reveal push debounced to one per group per window (`ROUNDS-PLAN.md` §4, §7.4) — gate: `npm run test:rules`, `npm run test --prefix functions`
