...
- [ ] `[claude-3]` **The recreation kit cannot address two prompt blocks that share an opening line, and D403 just created four of them.** `resolvePrompt` finds a block by `opens` matching its FIRST line, and `docs/COST-HUNT.md`'s hunter and reviewer blocks both begin `ultracode` — so the cost hunt's five Routines cannot be given manifest rows, and `node scripts/routines.mjs --plan claude-3 --missing` cannot print the five pastes the owner owes. Nothing depends on it today (no cost-hunt Routine exists yet), which is why it is filed rather than fixed inside a PR that is waiting on a merge. Done: `opens` matches the block's first NON-EMPTY line, or a `contains` alternative is added, with a case pinning two same-opening blocks resolving to different prompts; then the five cost-hunt rows join `routines/manifest.json` as `not yet` and `docs/RECREATE.md` is regenerated — gate: `npm run check:routines`, `npm run test:scripts`
...
- **WITHDRAWN 2026-09-06, not done — `[claude-2]` "the rules engine is hitting Firestore's 1000-expression ceiling on the answer-write rules".** Filed by this branch on 2026-09-04 from the ceiling message in `test:e2e`'s log and commented three times on #381. **D409 measured it and it was false**: the ceiling is reached only on a `set` over an answer that already exists, which evaluates the create and update rules against one budget — and the e2e takes that path three times a run *on purpose*, to prove the refusal. The app never does: its edit path is `updateDoc`, so every `setDoc` on an answer is a first write. Injecting **+400** copies of the anchor-length check into the create rule still lands a legitimate answer, so the green path is nowhere near the edge. The two assertions I called possibly-green-while-wrong are refused either way. Left here rather than deleted because the log line that produced this reading is still printed on every run, and the next reader of it should find D409 instead of re-filing.
