...
- [ ] `[claude-3]` Rounds step 0+1 — probe whether `string()` resolves in `firestore.rules` (the zero-padded `roundKey` is the fallback), then replace the reveal-history fan-out with one `orderBy("revealedAt","desc")` query that reads both the day-keyed and round-keyed id shapes (`ROUNDS-PLAN.md` §2.2, §7.1) — gate: `npm run test:unit`, `npm run test:rules`
- [ ] `[claude-3]` Rounds step 2 — the round replaces the day: `round`/`roundKey`/`roundPlayers`/`roundDeadlineAt` on `v2_groups`, the answer id `g_{gid}_r{n}`, the lead bound off the deduped group `get()`, `duelQFor` on the round, and the scan querying `roundDeadlineAt <= now` (`ROUNDS-PLAN.md` §2, §3.2) — re-check the pre-launch assumption in §11 first — gate: `npm run test:rules`, `npm run test --prefix functions`, `npm run test:e2e:all`, `npm run check:globals`
- [ ] `[claude-3]` Rounds step 3 — a 1v1 reveals on the second answer and a group on the last: `onV2AnswerCreated`'s duel branch gains the `roundPlayers` arrayUnion and one completeness read, `revealGroupDay` becomes `revealRound` and opens the next round in the same commit (`ROUNDS-PLAN.md` §3.1) — gate: `npm run test --prefix functions`, `npm run test:e2e:all`
