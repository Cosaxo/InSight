...
- [ ] `[claude-3]` Rounds step 4 — the late answer: `late: true` required by the rules when the reveal exists, no `guessIdx` with it, a server-only append to the reveal, excluded from the dims, the ledger and `duel-{qid}` (`ROUNDS-PLAN.md` §4) — gate: `npm run test:rules`, `npm run test --prefix functions`
...
- [ ] `[claude-3]` Rounds step 6 — the duel bank burst: raise `scripts/duel-budget.mjs`'s pool target and per-run cap and run the lane daily for a period, every farm hard rule and the 0.5 dedup floor unchanged (`ROUNDS-PLAN.md` §6.1) — gate: `npm run check:content`, `npm run check:neighbors`, `npm run check:figures`, `npm run test:scripts`
