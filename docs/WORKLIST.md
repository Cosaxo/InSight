...
- [ ] `[claude-3]` The facet and position item banks — the compass's 36 position items (verbatim from `design/standalone-2026-09-07/politics-deep.jsx`) into `content/tests.json` with `facet` + `invert` carried on the bank doc, the generator carrying both, `content-parity`'s K per level, `testItemMeta`'s id join (`VISION-2026-09-07.md` §2.1, §2.5); the Big Five's 120 facet items follow the owner row on their source — done: the eager delta quoted (~0), the seed appends and never re-keys — gate: `npm run check:content`, `npm run check:eager-content`, `npm run test:unit`, `npm run test:scripts`, `npm run test --prefix functions`
...
- [ ] `[claude-3]` The in-place questions — *Six questions place them →* on the compass and the same grammar per Big Five domain, each tap a `LIVE.vote` on the bank item, no device key (`VISION-2026-09-07.md` §2.2) `[owner]` — waits on the D121 row in `OWNER-LIST.md` § Decisions — gate: `npm run test:unit`, `npm run check:answer-shape`, `npm run check:purge`
