...
- [ ] `[claude-2]` The same for this account: the twelve theory prompts to `prompts/<lane>.md` on `axiom-theory` with the live Routine names (the message in `PROGRAM-RUNBOOK.md` § The other subscriptions), plus the doc sweep's and the night shift's stored prompts as fenced blocks on `main`; then `routines/manifest.json` pointed at each and `docs/RECREATE.md` regenerated. One PR on `main` and one commit on `axiom-theory` — gate: `npm run check:routines`, `node graph/check.mjs --all` on the branch
