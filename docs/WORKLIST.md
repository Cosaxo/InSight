...
- [ ] `[claude-3]` Draw arranged contests (`VISUAL-REQUESTS.md` § 2) once the owner's canvas is accepted and `AXIOMS.md` says operational — the offer, play and result rows on the daily tab in the duel cards' family, the sitting as twelve items of the logic overlay with the opponent and stake in its header, the consent line in the profile's General panel; transport is `src/v2/data/contest.ts` — gate: `npm run test:unit`, the smoke suites, `check:bundle`
- [ ] `[claude-3]` An e2e leg for the contest loop on the emulator — opt in → `runContestSweep` → accept both → start → submit both → settled, points moved, index rows right — beside `firestore-tests/e2e-v2-loop.mjs` — gate: `npm run test:e2e:all`
- [ ] `[claude-3]` The nightly contest fold (paper 8 §4.1, G1's roles): the stakes contrast per person from `stakesContrast` and the coupling between contest performance and the logic score on the BLIND pairings only, published like the logic norms with no uid — gate: `npm test --prefix functions`
- [ ] `[claude-3]` Enforce the mismatched-side budget (paper 8 §6) in the sweep: count each person's blind pairings inside the exposure window and stop offering blind draws to anyone over it `[ask]` — the cap is a design number
