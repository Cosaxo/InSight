...
- [ ] `[claude-1]` After the owner restores night shift B to `0 20,22,0,2,4 * * *` (`OWNER-LIST.md` § Clicks, D370): re-verify the trigger from `list_triggers` and rewrite §2's B row in `docs/ROUTINES.md` — five flows, the 22:00 and 02:00 audits back, and the model the click confirms, which no row has ever carried. Only this account can read that Routine, which is why the item is tagged rather than done in the PR that raised it — done: §2's row matches the tool response and §6's clock note about the two A-only hours is retired — gate: `npm run check:docs`
