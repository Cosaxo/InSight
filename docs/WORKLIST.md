...
- [x] `[claude-3]` The in-place questions — *Six questions place them →* on the compass and the same grammar per Big Five domain, each tap a `LIVE.vote` on the bank item, no device key (`VISION-2026-09-07.md` §2.2) — gate: `npm run test:unit`, `npm run check:answer-shape`, `npm run check:purge` — **not built: the owner ruled feed only, 2026-09-07 (D416)**
- [x] `[claude-3]` Rounds step 0+1 — probe whether `string()` resolves in `firestore.rules` (the zero-padded `roundKey` is the fallback), then replace the reveal-history fan-out with one `orderBy("revealedAt","desc")` query that reads both the day-keyed and round-keyed id shapes (`ROUNDS-PLAN.md` §2.2, §7.1) — gate: `npm run test:unit`, `npm run test:rules` — **built 2026-09-08 (D420's second amendment)**
- [x] `[claude-3]` Rounds step 2 — the round replaces the day: `round`/`roundKey`/`roundPlayers`/`roundDeadlineAt` on `v2_groups`, the answer id `g_{gid}_r{n}`, the lead bound off the deduped group `get()`, `duelQFor` on the round, and the scan querying `roundDeadlineAt <= now` (`ROUNDS-PLAN.md` §2, §3.2) — re-check the pre-launch assumption in §11 first — gate: `npm run test:rules`, `npm run test --prefix functions`, `npm run test:e2e:all`, `npm run check:globals` — **built 2026-09-08 (D420's second amendment), with step 3**
- [x] `[claude-3]` Rounds step 3 — a 1v1 reveals on the second answer and a group on the last: `onV2AnswerCreated`'s duel branch gains the `roundPlayers` arrayUnion and one completeness read, `revealGroupDay` becomes `revealRound` and opens the next round in the same commit (`ROUNDS-PLAN.md` §3.1) — gate: `npm run test --prefix functions`, `npm run test:e2e:all` — **built 2026-09-08 (D420's second amendment)**
- [x] `[claude-3]` Rounds step 4 — the late answer: `late: true` required by the rules when the reveal exists, no `guessIdx` with it, a server-only append to the reveal, excluded from the dims, the ledger and `duel-{qid}` (`ROUNDS-PLAN.md` §4) — gate: `npm run test:rules`, `npm run test --prefix functions` — **built 2026-09-08 (D420's second amendment)**
- [ ] `[claude-3]` Rounds step 5 — notifications, the owner's 2026-09-08 ask: `web/privacy.html` FIRST (a fifth kind, and *day* → *round* — `check:policy-claims` and `check:figures` refuse the send until the page says it), then the `turns` channel at importance 3, the one send site in the answer trigger (*your turn* to a member who has not answered, the reveal to one who has, never both), the per-recipient `pushAt` debounce naming the count, a group member nudged once per round, and the client's foreground suppression (`ROUNDS-PLAN.md` §7.4) — gate: `npm run check:policy-claims`, `npm run check:figures`, `npm run test --prefix functions`, `npm run test:unit`
- [x] `[claude-3]` Rounds step 6 — the duel bank burst: raise `scripts/duel-budget.mjs`'s pool target and per-run cap and run the lane daily for a period, every farm hard rule and the 0.5 dedup floor unchanged (`ROUNDS-PLAN.md` §6.1) — gate: `npm run check:content`, `npm run check:neighbors`, `npm run check:figures`, `npm run test:scripts` — **the regulator built 2026-09-08 (D420's second amendment); the lane's daily cadence is an owner row, since its Routine is on another account**
- [x] `[claude-3]` Rounds step 7 — world questions as duel content: the rules' second arm (a `daily`/`feed` question of an option-index type, the catalog still refused), `worldDuelPool` over the feed's core with even rounds drawing from it, the partner's public answers as the no-guess signal in a 1v1, and the reveal's third column (`ROUNDS-PLAN.md` §6.2, §0a) — gate: `npm run test:rules`, `npm run test:unit`, `npm run test:e2e:all`, `npm run test:scripts` — **built 2026-09-08 (D420's second amendment)**
- [ ] `[claude-3]` Rounds step 8 — the screens: the 1v1 and group card when a round is the unit, visual request 11 — the owner's Claude Design canvas first (D352), then extraction into `design/`, then the build (`ROUNDS-PLAN.md` §7.5) — gate: `npm run test:unit`, `npm run check:a11y`, `npm run check:tap-targets`, `npm run check:public-copy`
