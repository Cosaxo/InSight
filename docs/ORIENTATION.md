...
| [`VISION-2026-09-08.md`](VISION-2026-09-08.md) | The 2026-09-08 design measured against the tree — the current vision (D429): rounds reaching the prototype's demo twins (the live card was already built), the ruler saying Groups · 1v1s, the paid family gone from the prototype, the person's page as four tabs (Match · Answers · Together · Map) with a swipe, and a polish pass — 44 px slack, one reduced-motion switch, one timer for every `Lazy` card. Nothing built; two owner rows (the map's friends-only sentence, the buyer's room). Source extracted to `design/standalone-2026-09-08/` — a minified bundle, the README says how it is read | plan |
