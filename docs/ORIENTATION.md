...
| [`ROLES-PLAN.md`](ROLES-PLAN.md) | The 1v1 and group profile measured — the role matcher probed on simulated records (`scripts/roles-probe.mjs`): a fortnight cannot hold a name, the group instrument is one axis wearing three — and redesigned: the pair's reading and your role across pairs as two instruments, a group instrument over four dims that are four, every rate scaled so 50 is luck, a per-member ledger the reveal writes, the 1v1 questions' domain tag carried into the seed, the cast drawn. Nothing built; the owner's call is on `OWNER-LIST.md` and the screens are requests 5 and 6 | plan |
