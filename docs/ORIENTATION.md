...
| `check:fn-boot` | ci | What every Cloud Function loads before it can serve anything. `firebase.json` declares one codebase, so a container parses the whole of `functions/lib/index.js` on every cold start whichever single function it was started for — the server's version of the claim `check:eager-content` makes about the client's first-paint graph, which had no equivalent here. Two rules: a denylist of packages the app provably does not use (the Realtime Database client was in all 49 functions of a Firestore-only app, via the root `firebase-functions` barrel), and a shrink-only module count in `check:globals` rule 4's shape. Asserts the GRAPH, not the clock — the same tree timed 637 and 744 ms within a minute, and a gate that fails on a slow runner is one people re-run until it passes. Needs a built `functions/lib`, because the failure is transitive: no line in this repository names the denied package |
