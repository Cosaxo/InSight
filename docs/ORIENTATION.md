...
| [`OMIB-PLAN.md`](OMIB-PLAN.md) | The logic test on the Open Matrices Item Bank, scored for accuracy (D472) — what "accurate" means in three nested claims (the stimulus is what was calibrated; the score uses the calibration; the percentile names a real population), and the build that makes each true: 2PL θ by EAP with a per-person standard error in place of `k/25`, stratified seeded forms of 25 from the 218 that clear the quality floor, a θ histogram under an OMIB era stamp with Φ(θ̂) against the calibration sample as the labelled fallback, per-item exposure in the D62 ledger, and the one check that says whether the calibration transferred — observed solve rates against published difficulty, r beyond −0.8. Sequenced so the server half ships dark before any screen; adaptive selection is phase 4, and why. Two owner calls, both answered: practice mode (yes at D473, then no at D476 — the test is taken like an IQ test), and the two blocked hosts | plan |
...
| `design/logic-build-cell-2026-09-12/README.md` | The owner's *Build the Cell* upload of 2026-09-12 — the logic test's answer screen once OMIB's taker CONSTRUCTS the missing cell out of twenty shapes instead of tapping one of six tiles (D472, visual request 14). The first Claude Design canvas filed here rather than an InSight standalone, so the extraction differs: three assets kept of nine, the vendor four dropped. Records what the artboard decides, and the two things the build must fix — its shape order is not OMIB's element numbering (twelve of twenty differ, and OMIB's order is what its Rotation rule steps through), and its arrows point inward where the calibrated bank's point out |
...
| `check:omib` | ci | The committed OMIB bank against the published archive it was built from (D472) — codes read back out of the artwork and held against the spreadsheet's own, so the two independent sources must still agree. Also that the goal cell stays empty: the day the bank ships the answer in its pictures is a day to stop on |
