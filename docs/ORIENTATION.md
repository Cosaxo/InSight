...
| `src/v2/spec-index.js` | Imports every spec module for side effects. **The order is semantic.** Also exports the four lazy groups (`loadWorldFeed`, `loadMirrorTab`, `loadMapTab`, `loadOverlays`) | `src/v2/README.md` |
...
| [`OPS-RUNBOOK.md`](OPS-RUNBOOK.md) | The routines that keep the routine program honest — the platform probe, the roll call, the two shepherds, the production reader, the release recorder, the pulse responder — and the list worker that finishes `WORKLIST.md`. Contracts, models, canonical prompts and the wiring. Four of the eight Routines exist since 2026-09-02 (§5 has their ids); the PR shepherd, the probe, the pulse responder and the dependency shepherd do not yet | mixed |
