...
| `check:answer-shape` | ci | Every answer-creating write still carries `qid`, `answeredAt` and `anchors`, and the rebuild still reads them (D275). An answer without its anchors snapshot can never be re-cohorted — the profile is mutable, so the cohort it was cast in is gone |
