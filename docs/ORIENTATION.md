...
| `design/setup-sheet-2026-09-08/README.md` | The account-setup sheet, the screen straight after the front door (visual request 10), six artboards. The delivered file is readable but *templated* — the artboards repeat over a `states` array — so the README reads it back as the strings a build must match, and names the two departures from the request: **no Skip at all** (the primary is *Continue*, and the counter goes with it) and a birthday drawn as a **year rather than a date**, with what that costs the exact age measured — the one the owner overruled the same day, so the build takes the whole date in one `type="date"` row (D421) |
