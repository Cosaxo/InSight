...
| `routines/` | `manifest.json` — every Routine on every subscription as the thing to recreate it from: name, schedule, model, binding, where its prompt block is, state and id, with the retired ids an inventory may still carry. `docs/RECREATE.md` is rendered from it and `check:routines` holds the two to the runbook inventories. Edit the manifest, run `node scripts/routines.mjs --write`, commit both | `docs/RECREATE.md` |
...
| [`RECREATE.md`](RECREATE.md) | Every Routine on every subscription — name, schedule, model, binding, prompt block, state, id — with the one line that makes any session recreate what its account is missing, plus which file each of the six lists is and where the monitoring lives. Generated from `routines/manifest.json`; the owner's 2026-09-03 ask | tree |
| [`CONSOLIDATION.md`](CONSOLIDATION.md) | The same program on one or two subscriptions instead of three — what survives, what each retirement costs measured, what moves to GitHub Actions where no subscription is spent, and the seventeen prompts that exist only inside two accounts and die with them. Proposed 2026-09-04 at the owner's word; nothing in it is applied | plan |
| [`USAGE-REDUCTION.md`](USAGE-REDUCTION.md) | The other budget: the account usage the routine program spends against three subscriptions, measured off the sessions' own usage blocks — the context-times-turns mechanism, what each lane is priced at, the cuts taken on 2026-09-03 (§6, L1 and L2 applied here) and the levers only the owner can pull. Not the Firebase bill; that is `COST-REDUCTION.md` | mixed |
...
| `check:routines` | ci | `routines/manifest.json` parses, every prompt it names is a fenced block on this branch with no account placeholder left in it, every trigger id a runbook inventory table names is a Routine it knows, and `RECREATE.md` is what it renders — so a session recreating a Routine from GitHub gets the block the contract means |
