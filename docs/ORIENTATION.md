...
| [`VISION-2026-09-06.md`](VISION-2026-09-06.md) | The 2026-09-06 design measured against the tree — the current vision (D387), nothing built yet: ink on paper (`lens-paper` by default, boxless feed cards, result card and profile as hairline sections, the ballot one hairline row, figures drawing themselves in), a 12px microtype floor app-wide, the serif voice reaching the Circle/1v1 prompts and the Oracle's halves, the lens legends behind one ⓘ, the Patterns dial in the header — and, on the model side, the subscription seat split 08-26 §2.2 has waited on, drawn by the owner's own upload. Source extracted to `design/standalone-2026-09-06/` | plan |
