...
| `design/logic-build-cell-2026-09-12/README.md` | The owner's *Build the Cell* upload of 2026-09-12 — the logic test's answer screen once OMIB's taker CONSTRUCTS the missing cell out of twenty shapes instead of tapping one of six tiles (D471, visual request 13). The first Claude Design canvas filed here rather than an InSight standalone, so the extraction differs: three assets kept of nine, the vendor four dropped. Records what the artboard decides, and the two things the build must fix — its shape order is not OMIB's element numbering (twelve of twenty differ, and OMIB's order is what its Rotation rule steps through), and its arrows point inward where the calibrated bank's point out |
...
| `check:omib` | ci | The committed OMIB bank against the published archive it was built from (D471) — codes read back out of the artwork and held against the spreadsheet's own, so the two independent sources must still agree. Also that the goal cell stays empty: the day the bank ships the answer in its pictures is a day to stop on |
