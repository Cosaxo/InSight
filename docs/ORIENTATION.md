...
| [`COST-SHIFT.md`](COST-SHIFT.md) | The cost shift's contract: four Opus auditors a night — the device's reads, the server's writes, the data's shape, the bill against the model — each pushing a measured branch, and a Fable closer that reviews, adjusts and opens the PRs the owner merges. Designed 2026-09-06 (D393); the five Routines are the owner's to create in a web UI, and this row flips to *tree* when the register carries their ids | plan |
