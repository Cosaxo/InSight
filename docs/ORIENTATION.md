...
| [`OPS-RUNBOOK.md`](OPS-RUNBOOK.md) | The routines that keep the routine program honest — the platform probe, the roll call, the two shepherds, the production reader, the release recorder, the pulse responder — and the list worker that finishes `WORKLIST.md`, plus the dispatcher session they fire through. Contracts, models, canonical prompts, the dispatcher's charter and the wiring. Some of the Routines exist; the inventory says which, and what is the owner's to do | mixed |
