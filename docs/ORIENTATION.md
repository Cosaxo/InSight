...
| `check:monitoring` | ci | The alert chain from the log line a function emits to the policy that reads it, walked from **both** ends since D334 — a policy wired to a line nobody writes, and a line written to a metric nobody created, fail separately and neither direction can see the other's fault. Every link fails the same silent way |
