...
| [`VISION-2026-09-12.md`](VISION-2026-09-12.md) | The 2026-09-12 design measured against the tree — the current vision (D-2026-09-12d): the owner's `InSight_19` upload, titled *Doxa*. A name in a serif wordmark (the owner's to pick — §1 prices every place the name lives), the friends handshake read as D101's two rows both ways with one push (§2), a *Your friends* overlay and its four doors, the You stop's Map with a trail, Find, a resizable card and coach hints (§3), the person page's two leads (§4), the shell's leave animation and tap slack (§5). §0 the table, §6 the three owner questions (the name, *Groups · 1v1s*, whether anything stays private until you are friends), §7 the build order with what needs no answer first. Built the same day once the owner picked *Doxa* and *Groups · 1v1s* (D-2026-09-12d's amendment); the outward name and Q3 stay open | plan |
...
| [`OMIB-PLAN.md`](OMIB-PLAN.md) | The logic test on the Open Matrices Item Bank, scored for accuracy (D473) — what "accurate" means in three nested claims (the stimulus is what was calibrated; the score uses the calibration; the percentile names a real population), and the build that makes each true: 2PL θ by EAP with a per-person standard error in place of `k/25`, stratified seeded forms of 25 from the 218 that clear the quality floor, a θ histogram under an OMIB era stamp with Φ(θ̂) against the calibration sample as the labelled fallback, per-item exposure in the D62 ledger, and the one check that says whether the calibration transferred — observed solve rates against published difficulty, r beyond −0.8. Sequenced so the server half ships dark before any screen; adaptive selection is phase 4, and why. Two owner calls, both answered: practice mode (yes at D474, then no at D477 — the test is taken like an IQ test), and the two blocked hosts | plan |
...
| [`COST-COMPARISON.md`](COST-COMPARISON.md) | Doxa's bill against other apps'. Superseded in its conclusion by D129, kept for its method | past |
...
| `design/standalone-2026-09-12/README.md` | The owner's `InSight_19` upload of 2026-09-12, titled *Doxa* — a name, a friends list with a handshake, a Map with a trail and Find: the 38 modules that moved against every whole record and the two new ones, measured on the AST with the recipe the directory carries (the bundle has no hashes of its own); the stylesheet's 40 new classes; where the tree departs and why |
| `design/logic-build-cell-2026-09-12/README.md` | The owner's *Build the Cell* upload of 2026-09-12 — the logic test's answer screen once OMIB's taker CONSTRUCTS the missing cell out of twenty shapes instead of tapping one of six tiles (D473, visual request 14). The first Claude Design canvas filed here rather than an InSight standalone, so the extraction differs: three assets kept of nine, the vendor four dropped. Records what the artboard decides, and the two things the build must fix — its shape order is not OMIB's element numbering (twelve of twenty differ, and OMIB's order is what its Rotation rule steps through), and its arrows point inward where the calibrated bank's point out |
...
| `check:omib` | ci | The committed OMIB bank against the published archive it was built from (D473) — codes read back out of the artwork and held against the spreadsheet's own, so the two independent sources must still agree. Also that the goal cell stays empty: the day the bank ships the answer in its pictures is a day to stop on |
