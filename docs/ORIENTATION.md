...
| [`PROGRAM-PLAN.md`](PROGRAM-PLAN.md) | The owner's 2026-09-02 ask — run the project from six lists and a console, across three subscriptions — restated against what runs today, with one mechanism proposed per list, the new lanes (the axiom maker, the merge shift, a to-do doer per account, the console), the rule every session reads, and the questions the owner answers first. Nothing built | plan |
| [`PROGRAM-RUNBOOK.md`](PROGRAM-RUNBOOK.md) | The program plan as an ordered build list — the phases with their gates, the seed of each list, the contracts and canonical prompts for the axiom builder, the merge shift, the console and the to-do doers, and the account-side inventory. None of the steps is done and none of the Routines exists | plan |
