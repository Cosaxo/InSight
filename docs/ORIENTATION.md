...
| [`VISION-2026-09-06.md`](VISION-2026-09-06.md) | The 2026-09-06 design measured against the tree — the current vision (D387): ink on paper, a 12px microtype floor, the serif voice reaching further, the lens legends behind one ⓘ, the Patterns dial in the header — and, on the model side, the subscription seat split 08-26 §2.2 has waited on, drawn by the owner's own upload. Steps 1–2 are BUILT (D388 — the floor, and the Patterns instrument on paper); the dial, the feed's ground, the ballot row and the polish pass remain its §8 queue, and its two owner rows stand open. Source extracted to `design/standalone-2026-09-06/` | mixed |
