...
| [`OPS-RUNBOOK.md`](OPS-RUNBOOK.md) | The routines that keep the routine program honest — the platform probe, the roll call, the two shepherds, the production reader, the release recorder, the pulse responder — and the list worker that finishes `WORKLIST.md`. Contracts, models, canonical prompts and the wiring. Five of the eight Routines exist since 2026-09-02 (§5 has their ids); the probe, the pulse responder and the dependency shepherd do not yet | mixed |
