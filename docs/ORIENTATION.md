...
| `bigquery/` | The answer log's table schema (`answers.schema.json`, D439 phase A) — one row per ledger entry, the file `functions/src/log.ts`'s row is held to and `scripts/apply-bigquery.mjs` creates the table from; the dataset itself is a click, not a file |
