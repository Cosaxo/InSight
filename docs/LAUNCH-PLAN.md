...
1. ~~**Operator errand (networked machine):**~~ **half done 2026-08-23
   (D255).** `public/films.txt` and the regenerated
   ...
   not of musicians — so it needs a curation rule before a rerun, not
   another networked machine. Never hand-write keys (D15).
