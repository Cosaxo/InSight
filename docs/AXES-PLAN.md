...
(§2) — so the table carried six rows — and D347 added a seventh, for the
...
| Questions | The one write — daily, feed, learn, pulses (`docs/MIRROR.md` §1); duels are the Ties row's since D347. Core folds into the Mirror, tail does not, and absent means tail (D161) | `v2_users/{uid}/answers/{qid}` | Public |
...
| Interests | The least consolidated axis, three implementations: topic follows/mutes (`scenes.js`, device-local), catalogue favourites (D14–D17, D232/D266 — public answers keyed into committed catalogues), and the real follow graph (D101) — plus D163's interest model. Its theory lane since D347 | answers + `following` + local `insight.*` state | Mixed on purpose: favourites and follows public; the topic list and the interest model never leave the device (D163) |
| Ties (D347) | The relational axis — the 1v1 and group duel record (sealed answers, guesses, picks, reveals), the membership store and the follow graph: data whose unit is a pair or a group, not a person. `data/roles.ts` reads it as two instruments (D204), `duelRuns`/`groupPortrait` fold it, `duel-{qid}` is its cross-group signal (D40); the guess is the app's only second-person measurement, graded against a key | `v2_groups/{gid}` + `reveals/r{n}`, `v2_users/{uid}/following`, `v2_question_aggs/duel-{qid}` | Sealed-until: the answer is its author's until the round's reveal, which is public (D98); membership member-gated for the invite capability, not for privacy |
