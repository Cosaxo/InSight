...
| A standing charter for a relay session, given by automation alone | the program dispatcher, 2026-09-02 — refused the seed charter and a Routine-delivered confirmation as injected prompts | every Routine bound to it: the builder, the shift, the doers, the roll call, the improver | one human turn in the dispatcher session approving the charter (`OWNER-LIST.md` § Clicks); the alternative is creating the six lanes in the web UI, fresh session per run, which needs no dispatcher | open |
