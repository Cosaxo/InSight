...
| **MCP tools of every kind in a fresh-session Routine fire — MEASURED 2026-09-03, they are ABSENT** | the PR shepherd, fires at 15:55Z and 16:44Z | **everything the lane exists for.** No `mcp__github__*` means no merge and no comment; no `mcp__Claude_Code_Remote__add_repo` means no clone, so not even the `OPS-DIAG.md` fallback branch. A fire can think and nothing else | **re-create the Routine in the web UI** (claude.ai/code/routines) with the repository attached, prompt pasted from `OPS-RUNBOOK.md` §4, model `claude-opus-5`, cron `55 */3 * * *`; then disable `trig_01Ln6FDEipFzAghqJ777AL5j`. A Routine minted over MCP cannot pass tools it did not receive | open — **the one blocker left on this lane** |
