...
| Deleting a Routine from a session on this account | the planning session, 2026-09-03, rebinding the program lanes — three of five deletions allowed, the merge shift's and the list worker's refused by the auto-mode classifier with no stated reason | nothing — both were disabled instead (`update_trigger` was allowed) and re-created under new ids; two dead rows sit in the Routines page until deleted | delete `trig_01R5twbh48wfb9UfvVTANcdR` and `trig_017g4jkRVknNNccrVKfXWWS4` in this account's web UI (claude.ai/code/routines) | open |
| A standing charter for a relay session, given by automation alone | both dispatchers, 2026-09-02 — the program dispatcher here and the ops dispatcher on Claude 2 each refused their seed charter as an injected prompt, and then refused what a Routine delivered next — a confirmation fired into the one, the roll call's own 15:30 UTC firing into the other | on Claude 2, the ops roll call, the release recorder and the list worker; on this account nothing since 2026-09-03 — the program lanes were rebound to the planning session, which holds the owner's request in its own history and relays without a charter (`PROGRAM-RUNBOOK.md` 3.2) | one human turn in each dispatcher session adopting the charter (`OWNER-LIST.md` § Clicks) — the ops charter is now a section a session can verify against `main` for itself (`OPS-RUNBOOK.md` § The ops dispatcher, D353), which is what both sessions asked for; the alternative is creating the lanes in the web UI, fresh session per run, which needs no dispatcher | open |
