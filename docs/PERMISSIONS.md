...
| MCP connector tools (`mcp__github__*`) in a **fresh-session** Routine fire — UNMEASURED | the PR shepherd's re-creation, 2026-09-03; `create_trigger` warned *"this trigger stores no MCP connectors, so the sessions it fires will run without connector (mcp__<server>__*) tools"* | if it holds, the lane can clone, branch, push and run gates but **cannot merge or comment** — there is no `gh` CLI. It would push a branch and stop | the **first fire measures it** (2026-09-03 15:55Z; the run-log line says which tools it had). If they are absent, re-create the Routine from the web UI at claude.ai/code/routines, where connectors attach, and disable the MCP-made one | open — measured at the next fire, not before |
