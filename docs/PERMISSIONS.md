...
| Local git writes (`git merge`, `git checkout -B`, some `git merge-tree`) from the dispatcher-bound PR shepherd | the PR shepherd, every fire since 2026-09-02 20:55Z (run log #375) | **every renumber and every conflict resolution** — which is most of the lane. It fell back to GitHub's update-branch API, which takes no commit message, so it cannot carry the `shepherd:` subject step 4 checks for, and cannot resolve a conflict at all | run the lane in a **fresh session per fire** (`create_new_session_on_fire`) rather than bound to the ops dispatcher — an ordinary session has these; a session on this account ran all four merges, four renumbers and the battery on 2026-09-03 with no refusal. A Bash permission rule on the dispatcher session is the smaller alternative | open — the measured cause of the first shepherd stall |
