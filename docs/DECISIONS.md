...
## D131 · The Firestore region is a decision with a deadline, and it is written down before it expires
...
## D161 · The feed goes unbounded, and the Mirror gets a corpus of its own
...
- **Tail** — unbounded, personalized (D163), ordered by interest.
...
## D162 · Review at volume: the AI reads, and the human approves and audits
...
## D163 · The app learns what you are into, and the model never leaves the phone
...
uses it to order the **tail** (D161), and keeps it **on the device**. The
...
only (D161). The model never leaves the device.
...
## D164 · The revenue paths, re-derived against an unbounded feed
...
falls out of D161: paid questions inside the Mirror's corpus would make
...
D161 retires it. It was never one of the load-bearing three.
...
D163 makes it strictly better, because the device now holds an interest
...
answers accumulate. D161–D164 exist to produce more answers sooner, which
...
## D170 · Build 18's pre-flight: the record was written and the number was not

**Decided:** 2026-08-16 · **Status:** binding · Sixth release pre-flight
(D130, D142, D143, D153, D158). Bumps `appBuild` 17 → 18, and commits a
`Package.swift` that has been out of sync with the tree's own dependency
since dependabot. **The bump is the finding here** — three pre-flights
running had found nothing to do, and this one does.
...
| --- | --- | --- | --- | --- |
| total JS | 2255 KB | 2278 KB | **2284 KB** | 2285 |
| eager graph | 969 KB | 961 KB | **964 KB** | 978 |
| chunks | 72 | 75 | 75 | — |
...
**The total's headroom has gone 10 → 7 → 1 KB across three release
pre-flights.** D158 wrote that the next feature of any size would meet the
total; it did, twice, and what is left is not a margin — 1 KB is inside
the rounding of a single lazy chunk. The next commit that adds anything at
all trips this gate, and it will trip on the release path, where D144 put
it deliberately. That is a raise-or-trim decision with a history (2265 →
2285 at D156) and it belongs to whoever makes the next change, not to a
pre-flight that changed no code. The eager graph is the half that is not
in trouble: 964 against a constant D144 and D152 both declined to move,
14 KB of room.

### Measured, not asserted

1175 client tests (77 files), 214 function (8), 183 script (10), 90 rules
(2); all three e2e suites green — the loop, erasure and moderation, each
exiting 0. `lint`, `tsc -b`, and 26 `check:*` gates. `check:globals`
reports **409** cross-module references across 42 files, at its baseline
and unmoved: build 18's payload added no coupling.
...
Eleven commits, and **nothing shell-facing**. Between run 24's commit and
this tree the only diff under `ios/`, `android/`, `capacitor.config.ts` or
either `package.json` is the build number and one `build:dogs` script
line; `package-lock.json` is untouched, so no dependency moved. A pure
JavaScript payload, the same shape as build 16 — the archive step has no
new native surface.

The payload: the **dogs domain** (554 breeds on minted append-only keys,
its first card and the shared key-set writer), **D166**'s third tab
adopted on trial, **D167**'s every-surface-ships-with-its-backend rule,
**D168**'s refusal to assert born-or-built, and **D169**'s Mirror folding
once per change rather than once per render. Forty-one of the 69 changed
files are `design/standalone-v28`, a prototype extraction that ships
nothing.

**The store filing does not move.** Nothing here collects anything new — a
dog-breed answer is an answer, already declared as Other User Content — so
`docs/STORE-FORMS.md` stands as D141 left it and `check:store-forms`
passes unchanged.
