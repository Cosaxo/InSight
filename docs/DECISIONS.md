...
## D155 · The test surfaces stop describing a crowd they never counted
...
### 7 · The cost, stated because it is nearly the whole of what is left

The eager graph goes **969 → 975 KB against a ceiling of 978**. Four of
those six kilobytes are `data/similarity.ts` arithmetic reaching three
eager profile surfaces; the other two are `type-marks.jsx` importing
`typeMix.ts`, measured by removing that one edge (973 KB).

`MAX_EAGER_KB` is not raiseable — the doctrine above it says so twice and
this change does not raise it — so the next change to an eager surface
has 3 KB, not 9. The way out when it binds is the one the ceiling has
always implied: move a profile surface behind a dynamic import, starting
with `TypeIndexSheet`, which is a tap-to-open overlay reached through a
global and is eager only because `TypeMark` shares its file.
