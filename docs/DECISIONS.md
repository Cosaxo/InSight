...
## D25 · The spec layer's dead render code is deleted; the one toolkit is kept

**Decided:** 2026-07-31 · **Status:** binding

The port carried forward render code from prototype generations the v15
reference no longer has. None of it was reachable: **973 lines removed,
14 KB off the built bundle**, no behavioural change and no gate moved.
...
the spec layer, `tsc -b`, `check:a11y` unmoved at 19, `check:bundle`, and
`test:unit` at 193/193 — including `smoke.test.jsx` and `smoke-live.test.jsx`,
the only guards here that actually execute a render.
