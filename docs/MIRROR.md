...
| **Explore** | **live** | `divergence` across the six breakdown dims, against the GLOBE on every stop — its buckets are cuts of everyone and its sentence ends "same as everyone", so it reads `LensQuestion.all` rather than the stop's cohort (D170). The v18 test-pole axis is the one part with no source *here*, and its status changed at **D330**: test results ARE published cells now, just not in `agg.by`. The nightly sweep writes a per-question cube onto `v2_question_traits/{qid}` keyed on every instrument's matched type, its axis bands and the logic band, and the who-voted sheet draws all of it (`ui/LiveBreakdownPanel.tsx` via `LIVE.loadTraits`/`traitsFor` and `data/traitDims.ts`) — exact, over every answer, retroactive, replacing D146's 200-voter client sample. If Explore ever takes the axis it should read the cube through `data/traitDims.ts` rather than grow a second way to type people; the only work is that the cube is a second document and this lens currently folds `LensQuestion` alone. Its chips and its sentences printed the raw bucket KEY until D125 — a country row read "NO" — and now resolve through `ui/cohortLabels.ts`, the same one a feed card's breakdown sheet uses |
