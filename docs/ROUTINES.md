...
| InSight night shift B | `trig_01GNe14hPrZcYzXkFHjPH2bW` | `0 20,22,0,2,4 * * *` — **five** flows; 20:00, 22:00, 00:00 and 02:00 audit at 95 min, 04:00 closing at 110 min. **Re-verified from `list_triggers` 2026-09-07, `updated_at` 2026-09-05T21:17:55Z.** This row said `0 20,0,4 * * *` and "three", citing a 2026-09-03T14:28:12Z cut — the live record's own `updated_at` is two days LATER, so the schedule was set back to five after the row was written and nothing recorded it. Corroborated from inside the lane: a three-flow schedule cannot fire at 22:00, and the 2026-09-06 night fired at 20:00, 22:00 and 00:00. Who set it back, and whether the L1 cut should be re-applied, is on `OWNER-LIST.md` — a schedule is not a night shift's to change | worker session `session_01M9cvEjdQmWYjgrWvaoXiK9` | `nightb-YYYYMMDD` — anywhere a verified defect is | never merges; never pushes `main` or `night-*` |
