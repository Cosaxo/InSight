...
created for D480 (`trig_01NsFZGXqdFTFi8JACy81gNL`, `0 12 * * 1`, first
fire 2026-09-14), verified from `list_triggers` in the creating session
before this line was written, per rule 3 and CLAUDE.md's "in the same
PR". The two disabled are unchanged. It is the seventh content lane and
...
| InSight learn lane | `trig_01TckXyab4zPHT4NSfSqPjku` | `0 9 * * *` — daily 09:00 (Mon + Thu until 2026-09-08; re-paced on the owner's word, D428 amendment — half an hour before the feed lane, the spacing the two already had on Mon/Thu). New id: the prompt still carried the old hard rule 3 (*"Never create a field or subject"*) and *"twice weekly"*, `update_trigger` refuses a prompt edit into a session that is not the caller's own, so the D148 mechanism — created and verified first, `trig_01Qguc3PyigsW7RvQLvC6X5G` deleted after | same dev session | `claude/learn-cards-<date>` — `content/learn-questions.json` | self-merge (D212) |
