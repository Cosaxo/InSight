...
**Status: plan notes — proposes; nothing here is built.** Written
...
real fold is a one-line change and its own note.
...
**The tag reaches the seed.** `gen-v2content.mjs` emits the source's
...
1. **S · The day's kind and the honest fold.** `gen-v2content.mjs`
...
2. **S · Reading the room.** `LiveDuelPanel` runs the answer→guess
...
- **The guess on group days.** One more tap a day for every group
  ...
  simply carries no `guessIdx` and does not count.
