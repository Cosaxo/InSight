...
- They arrive through a **reviewed paid path** — a human contract when
  this was written, the D313 self-serve loop since 2026-08-26
  (automated review, Stripe checkout, the webhook writes the question
  live) — never generated by the question farm (its hard rule 6 exists
  to keep this inventory from being given away as free filler).
...
page anyone can open, which the answered card copies the address of (the
in-app buyer's room did too, until D430 took it out with the rest of the
paid family) — and `sponsor` as a provenance source `check:quality` holds in both
