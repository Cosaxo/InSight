...
| `ITMS-90683` emailed *after* a successful delivery | A purpose string is missing for an API some linked package references, whether or not this app calls it. Fix the plist, not the caller — `npm run check:ios-location` and D103. The delivered build stands; the fix rides the next one |
