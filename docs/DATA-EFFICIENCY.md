...
morning; §5 is the build order, approved by the owner the same day (D440)
...
> The owner chose *live* (D440 amendment); the trigger writes the entry in
...
### 2.2 · The nightly sample carries names — 72 → 1 reads per user-day · **BUILT 2026-09-08 (runbook 2.1–2.3)**

> Built as described, with one shape added: a profile trigger
> (`functions/src/profileFanout.ts`) moves a changed name or score into
> every row the account holds the moment the profile changes, because
> "a row it does not touch keeps its name until it is refolded" below
> would have been a visible regression against the seven-day cache for
> anyone who renames. The runbook's 2.1 has the arithmetic. **And a
> budget (2026-09-09, `COST-EXPOSURE.md` §8):** the trigger is the
> largest amplifier of one client write in the deploy — an account's
> answers in reads plus a read and a write per sample holding its row,
> per stamp change, and a profile takes a write a second — so it fans
> out at most three times an hour per account and leaves a `pending`
> marker past that, which the nightly pass heals from the profile as it
> is then. A real rename is a handful of times per account lifetime;
> the last of a burst still lands within a day.
...
### 2.3 · The who-voted sheet draws from the sample — 60 → 0.15 reads per user-day · **BUILT 2026-09-08 (runbook 2.4), for the cold question**

> The hybrid this section describes is what shipped, with the claim kept
> exact: a question whose live tail fills its cap (today's daily, at any
> real size) reads the full live list as before, so "the newest 200" is
> never fifty of today over a hundred and fifty of before. The model
> charges that share at `B.sheetOpensHot` (0.7): the sheet term is 60 →
> 46, not 60 → 0.15, until the owner rules on the cheaper sentence
> (`OWNER-LIST.md`).
...
### 2.9 · The city-scoped Kindred pass — twelve live fan-outs the model does not see · **BUILT 2026-09-08 (runbook 2.5): the city-scoped sample**

> The first of the two shapes below shipped: `city-{qid}~{city}`
> documents from the same nightly fold, bounded per night rather than by
> a size floor (the runbook's 2.5 has why a floor cannot accumulate), and
> the pass reads one document per question. The model now carries the
> term (`cityKindred`, 0.36 reads per user-day where it was ~72 unseen).
...
- **The nightly pass holds the day — the fifth, found on the other
  ...
  holds on disk — and D441 puts the direction to the owner.
