...
| Over-time ("pulse") questions | **BUILT (D139; roster of five at D203)** — the verdict was "the strongest idea on the list" | L | One answer per question is structural (`aid == qid`). The duel answers' day-keyed id (`g_{gid}_{day}`) is the working precedent to copy. |
| Mood as the first pulse question | **Decide separately** — still open | S on top | Half the blocker has since moved: the store forms' Health row went YES with the pulse roster (D166 §3, D203). What remains an owner decision is the public per-person series, which no pulse makes today. |
| Types focus | **Tier 1 BUILT (D141; every-instrument switch at D202); tier 2 is still the real decision** | M / L | Tier 1 rides data that is already public. Tier 2 (type as a breakdown dim) amends the standing "a test result is never a breakdown dim" claim (D8, `docs/data-inventory.md`). |
| Height | **BUILT (D140)** — a banded anchor, the age-band pattern | M | Bands, never centimetres, server-side; the device folds and discards the number the way `locate.ts` folds coordinates. |
...
| Genetics | **Refuse for now** (the owner already said "future") | — | Art. 9 data cannot live in a world-readable profile; an acceptable version is a separate consented tier, i.e. a different product posture — which is now planned as exactly that, in `AXES-PLAN.md` §4 and `AXES-RUNBOOK.md` phases 3–4, each phase behind its own owner record. |
| Suggest a question | **BUILT (D138)** — and then re-doored: D288 §1 deleted the community board and made the composer the PAID door, self-serve since D313 | M | The UI, the dedup gate and the `community` provenance source all existed; D138 added the collection, callable and human gate this row asked for. |
