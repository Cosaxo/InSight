...
- `seedContentV2` (callable; emulator or SEED_ADMIN_UIDS allowlist) — mirrors `/content` question banks
  into `v2_questions` (500 docs, stable ids `daily-000`, `feed-<id>`,
...
everything. The question bank (500 docs) caches in localStorage keyed by
...
- `npm run test:rules` — 69 rules tests (Firestore + Storage; the v2
