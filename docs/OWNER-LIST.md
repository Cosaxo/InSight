...
- [ ] **Re-create the PR shepherd in the web UI — the one blocker left on that lane.** claude.ai/code/routines → New, **repository `Cosaxo/InSight` attached** (this is the part that fixes it), model `claude-opus-5`, cron `55 */3 * * *`, prompt pasted from `OPS-RUNBOOK.md` §4. Then disable `trig_01Ln6FDEipFzAghqJ777AL5j`. **Measured, not guessed:** two fires on 2026-09-03 (15:55Z, 16:44Z), both instructed to log unconditionally, left no comment, no merge, no branch and not even the diag-branch fallback — a Routine minted over MCP gives its fired sessions no MCP tools at all, so the lane can think and nothing else. If the UI offers to install the Claude GitHub App, accept: that also wires the `pull_request` event triggers, so a label is acted on in minutes instead of up to three hours. *Source:* `PERMISSIONS.md`; run log #375; PR #386.
...
<!-- console:begin -->
- [ ] 15 PR row(s) and 2 branch row(s) waiting for a tick in `docs/MERGE-LIST.md` § Open (2026-09-03).
<!-- console:end -->
