...
- [ ] **Where the six program Routines should fire from** — today the five fire into the planning session, which relays each into a fresh session: it works (the builder's first run accepted its prompt at once) and it is the shape D359 measured as the bill — every firing replays that session's ~750k-token context before one relay call, against $1.91 for a fresh-session fire. The fix is six pastes: create them in this account's Routines page from `node scripts/routines.mjs --plan claude-3` (the `web_ui` fields; the console keeper is the sixth), and the relay-bound five are deleted the same day. Keeping the relay costs roughly a metered Fable turn per firing, fourteen a day at full cadence. *2026-09-04: the account now carries four — the shift was deleted and the builder re-paced to once a day from the Routines page — so the relay wakes are six a day.* *Source:* `PROGRAM-RUNBOOK.md` 3.2, § Platform measurements; D359.
- [ ] **The merge shift's cadence while nothing is approved** — nine firings a day, each an Opus session that finds no `approved` PR and says so. The relay now gates the spawn (no approved PR, no session), so the cost is the wake alone; the owner's dial is `15 5,7,9,11,13,15,17,19,23 * * *` → three passes (`15 7,15,23 * * *`) until the tick flow has merged something, or the nine as written. The builder's first run also spent the account's five-hour window to its limit alongside this session: three builder runs a day and the relay wakes do not both fit in one bucket. *2026-09-04: acted on from the account's Routines page — both merge-shift Routines deleted, the builder at `30 6 * * *`; re-creating the shift is `RECREATE.md`'s row when the tick flow is wanted.* *Source:* `PROGRAM-RUNBOOK.md` § Platform measurements; `ROUTINES.md` §5 The account budget.
...
- [ ] **Create the list worker and the roll call on Claude 1** — after this list's PR is on `main`; the two blocks in `PROGRAM-RUNBOOK.md` § The other subscriptions, or the one message there to a Claude 1 session — or the one line in `RECREATE.md` § In any session, which creates both from `routines/manifest.json` and records the ids. *Source:* `PROGRAM-RUNBOOK.md` 5.1.
- [ ] **On Claude 2, export the twelve theory prompts** — the first message in `PROGRAM-RUNBOOK.md` § The other subscriptions; this account cannot read them. `RECREATE.md` § Not yet recreatable lists what waits on it, and `WORKLIST.md` carries the same item tagged `[claude-2]`. *Source:* `PROGRAM-RUNBOOK.md` 4.1.
...
- [ ] **Delete the two disabled Routines on this account** — `InSight merge shift` (`trig_01R5twbh48wfb9UfvVTANcdR`) and `InSight list worker (Claude 3)` (`trig_017g4jkRVknNNccrVKfXWWS4`), in this account's Routines page (claude.ai/code/routines). Both were re-created 2026-09-03 bound to the planning session; the classifier refused deleting the old ones from a session, so they are disabled and fire nothing — tidiness, not urgency. *Source:* `PROGRAM-RUNBOOK.md` 3.2; `PERMISSIONS.md`.
- [ ] **Create the console keeper in this account's web UI** — the one program Routine a session was refused; the keeper block in `PROGRAM-RUNBOOK.md` § Canonical prompts, `45 5,17 * * *`, `claude-sonnet-5`, repository attached. The five boxes are also the `web_ui` fields `node scripts/routines.mjs --plan claude-3 --missing` prints (`RECREATE.md`). *Source:* `PROGRAM-RUNBOOK.md` 3.3; `PERMISSIONS.md`.
...
- [x] **Approve the program dispatcher's charter in its own session** — superseded 2026-09-03: the five program Routines were rebound to the planning session (`session_013V91NnDHMjjLSGYxzTEsnw`), which holds the owner's request in its own history and relays without a charter; the dispatcher session `session_01THJsyLkHr1aJskpnhahwuf` has nothing bound to it. *Source:* `PROGRAM-RUNBOOK.md` 3.2, § Platform measurements.
