...
- [ ] **The theory lanes' second set** — `PROGRAM-PLAN.md` §4.3 prices it at twice the month the twelve cost today, on the bucket no session here can read. *Source:* `USAGE-REDUCTION.md` §4.3, L9. **WITHDRAWN 2026-09-04 (D364)** — twelve more Routines at the opposite parity, priced at about the same again per month, on the program the owner has just stopped. It was recommended because it was the only reading that added capacity; capacity was not what was short.
...
- [ ] **Disable the twelve theory lanes — the only thing that actually stops them (D364).** Your word today was *"lets disable the axiom theory lanes they have not work as i hoped"*, and the record is written; the switches are not this account's to flip. They live on **Claude 2** (`a571fec5-…`) and a session on any other account is refused — probed 2026-09-04, `update_trigger` on the genetic lane returned *"the requested resource was not found"*. **Until you flip them they keep firing**, at ~3.43 runs a day × $24.44 ≈ **$84 a day**. In that account's Routines web UI, set each to disabled — **do not delete**, the run history is ten days of provenance and re-enabling is one field: `trig_01P1aDKgDhab3yLeCrYn3TAt` (review) · `trig_01Vx4tmhq3EVwySCjSESjrrW` (genetic) · `trig_01AopNS2HAVVHFYk99w7oJv7` (body) · `trig_01JeVZmgC9FB78L5VRxGQJ9L` (questions) · `trig_01URyaqWz9WgLdRJVDn6z8hX` (tests) · `trig_01PjG2bW3zK3GTgnfaYTjQky` (ties) · `trig_01HUHXnMT6xAiEaurLxeBJNq` (interests) · `trig_01VDccEWW215SDJPE3ujHciL` (database) · `trig_014HZHQYSpjc4xQGfbyAgjXw` (map) · `trig_01AsWK9g327DuHD6XatbBAmR` (pattern) · `trig_016uPKLAXGriwC7ukQyRRmUG` (graph optimizer) · `trig_017ZfLe6VNmVGZ677qqvkqgm` (central). *Source:* D364; `ROUTINES.md` § The theory lanes.
...
<!-- console:begin -->
- [ ] 19 PR row(s) and 4 branch row(s) waiting for a tick in `docs/MERGE-LIST.md` § Open (2026-09-04).
<!-- console:end -->
