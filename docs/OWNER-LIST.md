...
- [ ] **The merge shift's nine daily firings** — *you answered 2026-09-04: "remove the merge shifts"; the tick is yours to make.* Both Routines deleted (`trig_01KSwCf4WcnQE6Uh6udeNAxn`, live and firing until 05:16 that morning; `trig_01R5twbh48wfb9UfvVTANcdR`, its disabled predecessor), and the tick on `MERGE-LIST.md` now writes `merge-when-green` itself, so the PR shepherd is the whole path after your approval. What went with the lane is the second read of the whole diff — D363 § What it costs. *Source:* `USAGE-REDUCTION.md` §4.2, §9; D363.
- [ ] **The theory lanes' second set** — `PROGRAM-PLAN.md` §4.3 prices it at twice the month the twelve cost today, on the bucket no session here can read. Read against D363's weekly cadence rather than the old one: the twelve now cost ~$42 a day, and *"reduce the theory production"* is the direction the owner gave on 2026-09-04. *Source:* `USAGE-REDUCTION.md` §4.3, §9, L9.
...
- [ ] **Re-pace the twelve theory lanes to one run a week** — the account half of D363, and the only half a session here could not do: those Routines are on the axiom dispatcher's subscription, and `list_triggers` returns only the calling account's. Twelve `update_trigger` calls, **schedule only** (a prompt edit on a Routine bound to another session is refused, and none is needed): genetic `2 9 * * 1` · body `2 10 * * 1` · questions `2 11 * * 2` · tests `2 12 * * 2` · ties `2 13 * * 3` · interests `2 14 * * 3` · database `2 8 * * 4` · map `2 9 * * 4` · pattern `2 10 * * 5` · graph optimizer `2 11 * * 5` · central `2 12 * * 6` · review `2 2 * * 0`. Trigger ids in `ROUTINES.md` § The theory lanes. Worth ~$31 a day. *Source:* D363; `USAGE-REDUCTION.md` §9.
- [ ] **Amend `CHARTER.md` §10 on the `axiom-theory` branch** — the canonical inventory still carries the every-other-day crons D359 replaced on 2026-09-03, and no routine may amend a lane contract (`AXES-PLAN.md` §10). The paragraph and the twelve values to paste are in `AXIOM-THEORY.md` § The corrections, item 2. *Source:* D363; `AXIOM-THEORY.md` § The corrections.
- [ ] **The axiom builder's stored prompt still says "three times a day"** — its schedule is `30 6 * * *` since 2026-09-04, and its contract (`PROGRAM-RUNBOOK.md` § The axiom builder) says once a day and outranks the summary, so nothing is broken; the sentence is just wrong. A prompt edit is refused from a session that does not own the binding, so it is one line in the web UI when you are next there. *Source:* D363.
...
<!-- console:begin -->
- [ ] 17 PR row(s) and 4 branch row(s) waiting for a tick in `docs/MERGE-LIST.md` § Open (2026-09-04).
<!-- console:end -->
