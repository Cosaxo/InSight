...
- [ ] **The paid door's shape — and its window closes the moment you submit.** `STORE-CUT-PLAN.md` is plan only and recommends shape A (the door is not in the app; buying lives on the web, the app keeps the results room). Two facts make A nearly free *today* and both expire at submission: there are **zero sales** (every `booked` array in `content/pricing.json` is all zeros, verified 2026-09-04) so nothing migrates, and the app has never been reviewed, so the door never has to be *removed*. The risk is 3.1.1 anti-steering: the category argument is strong (3.1.3(e) forbids IAP for campaign purchases) and the app-shape argument is weak — a consumer app with a €320 B2B door in its profile tab is Meta's "Boost Post" shape, and Meta lost it. The link-out entitlement likely does not reach us: Norway is EEA, not EU. Keeping the door is a legitimate answer; arriving at 6.2 without choosing is choosing it by default at the moment it costs most. *Source:* `STORE-CUT-PLAN.md`; `LAUNCH-RUNBOOK.md` 6.0; D363.
...
- [ ] **Decide the two App Check debug token values, and put them where they live** — the only half of runbook 1.4 that is still yours since D363. Generate a UUID4 for CI and put it in GitHub → Secrets → `APPCHECK_DEBUG_TOKEN` (the name `screenshots.yml` already reads); generate a second, different one for your own browser and put it in your local `.env` as `VITE_APPCHECK_DEBUG`. **Two values, not one** — a token is a credential, and one shared between a laptop and a public CI log is revoked in both places at once when either leaks. Then Actions → **App Check** → `register-debug-token` registers the CI one, and `report` shows you both by display name; no console, and no token ever passes through a run log. Registering yours is the same dispatch with your app id, or the console if you would rather not put it in a secret at all. *Source:* `LAUNCH-RUNBOOK.md` 1.4; D337; D363.
