...
- [ ] **Whether to apply the usage levers, and which** — this account read `allowed_warning` on its seven-day window 26% of the way into it (2026-09-03). `USAGE-REDUCTION.md` §3 ranks ten levers; nine of them keep every lane's output and change only prompt text, a cadence or a model. Cadence is yours (the register's rule 5), so none is applied. *Source:* `USAGE-REDUCTION.md`.
- [ ] **Two night shifts, or one?** They audit the same tree on the same night from two accounts, and the collision machinery is a cost of running both. Dropping one frees ~430–490 minutes a day on one bucket — the largest single cut available, and a product decision rather than waste: the output merges. The recommendation is to keep both, apply L1 (audit once, fix four times) first, and re-read the band after one window. *Source:* `USAGE-REDUCTION.md` §4.1.
