...
- [ ] **Set the four fire secrets** — `ROUTINE_PULSE_FIRE_URL`/`_TOKEN`, `ROUTINE_RELEASE_FIRE_URL`/`_TOKEN` — when the pulse responder and the release recorder get their API triggers. **The release half has now cost a delivery, measured:** build 29 went out on 2026-09-02 (run 50) and `ios-release.yml`'s step 18 fired nothing — its log reads *"release recorder not wired (ROUTINE_RELEASE_FIRE_URL / ROUTINE_RELEASE_FIRE_TOKEN unset) — record this delivery by hand"* — so the build was unrecorded until build 30's pre-flight read the run list two days later, the fifth time that has happened and the first with the fix already in the tree. The step is never-red on purpose, so nothing else will ever raise this. *Source:* `OPS-RUNBOOK.md` §2.4; `PERMISSIONS.md`; D364.
