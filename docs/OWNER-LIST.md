...
- [ ] **Whether to apply the usage levers, and which** — this account read `allowed_warning` on its seven-day window 26% of the way into it, and `get_session` prices the lanes: night shift B **$594.66 over two nights**, all six content lanes **$160.70 since 07-30**. `USAGE-REDUCTION.md` §3 ranks ten levers by measured dollars saved; none reduces output unless its row says so. Cadence is yours (the register's rule 5), so none is applied. *Source:* `USAGE-REDUCTION.md`.
- [ ] **The two silent lanes — disable, or make them report?** The nightly algorithm improver ($52.64 for the 09-03 run) and the DB scalability lane ($3.03) have left **no branch on origin and no PR ever** since they were created 2026-08-27. ~$1,600 a month for output nothing in the repository can see. Recommendation: disable now, re-enable behind a rule that makes a run say on the log why it pushed nothing. *Source:* `USAGE-REDUCTION.md` §1, §4.2, L2.
