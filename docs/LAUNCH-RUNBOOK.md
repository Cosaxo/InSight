...
`check:store-listing` and `check:versions` pass; the daily bank is at 126
questions of 616 seeded; the production backend is deployed. **Measured
...
- [x] **0.1 Seed the production question bank — CLOSED, measured
      ...
      **exactly 600 documents and zero `test-cognitive-*` docs**: the
      ...
      616 questions land in `v2_questions` — idempotent and, since D34,
      cheap to repeat.
      ...
      **It is unticked on purpose, and still is.** That run wrote **389**,
      and the bank is **616** after the K=5 test expansion, D103's
...
- [ ] **3.2 TestFlight with ten testers, not five.** The public mirror
      ...
      **Expect your own answer to be the whole crowd, and expect to see
      ...
      working, not a leak — the 616 seeded questions are live regardless.
