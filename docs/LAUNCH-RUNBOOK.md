...
questions of 1538 seeded; the production backend is deployed. **Measured
...
- [x] **0.1 Seed the production question bank — CLOSED, measured
      ...
      **exactly 600 documents and zero `test-cognitive-*` docs**: the
      ...
      1538 questions land in `v2_questions` — idempotent and, since D34,
      cheap to repeat.
      ...
      **It is unticked on purpose, and still is.** That run wrote **389**,
      and the bank is **1538** after the K=5 test expansion, D103's
...
- [ ] **3.2 TestFlight — two testers is enough to start. THE "TEN" WAS
      ...
      **Expect your own answer to be the whole crowd, and expect to see
      ...
      working, not a leak — the 1538 seeded questions are live regardless.
...
- [ ] **5.17 Stand the budget's wire up — one click after the deploy
      ...
      budget's wire). Since D465 the same function is also the hard stop
      — 5.18 is the grant that arms it.

- [ ] **5.18 Arm the hard stop — one IAM grant after the deploy that
      carries D465, then the 300 % mail (2026-09-12).** `functions/src/
