...
`check:store-listing` and `check:versions` pass; the daily bank is at 144
questions of 1577 seeded; the production backend is deployed. **Measured
...
- [x] **0.1 Seed the production question bank — CLOSED, measured
      ...
      **exactly 600 documents and zero `test-cognitive-*` docs**: the
      ...
      1577 questions land in `v2_questions` — idempotent and, since D34,
      cheap to repeat.
      ...
      **It is unticked on purpose, and still is.** That run wrote **389**,
      and the bank is **1577** after the K=5 test expansion, D103's
...
- [ ] **3.2 TestFlight — two testers is enough to start. THE "TEN" WAS
      ...
      **Expect your own answer to be the whole crowd, and expect to see
      ...
      working, not a leak — the 1577 seeded questions are live regardless.
...
- [ ] **5.11 Install the BigQuery mirror WITH the first real users — not
      ...
      **Superseded 2026-09-09 (D447 phase A, `LOG-FIRST-RUNBOOK.md`):** the
      ...
      Both were clicked 2026-09-10; what stays open is the trigger's two
      roles, for the account the functions RUN AS rather than the one the
      script printed, which *Observe production* now reads
      (D-2026-09-12a).
