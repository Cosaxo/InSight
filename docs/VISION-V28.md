...
| **What moves together** (cross-test threads on the profile) | **BUILT — and half of it was not real** ([D393](DECISIONS.md#d393--nothing-in-the-app-is-invented-the-trait-webs-usual-pattern-and-the-maps-knowledge-radius-were-and-the-dailys-count-is-not)) | none for the dots; the *usual pattern* needed a measurement this row said it did not | The dots were always your own results. The direction each pair was laid against was the authored sign table, drawn in a live build as a finding about people nobody had counted. Since D393 a live build measures each pair over the cached voter sample (thirty people, two standard errors) and draws no pair it cannot state — absent on a young install, arriving with the data. |
...
| --- | --- |
| **Trait web** (§7.9, `trait-links.js`) | The DOTS, yes: the viewer's OWN results, which the device already holds. The *direction* each pair is laid on was authored, and this row read as licence to draw it in a live build; D393 made the live build measure it over the cached voter sample instead — no new read class (the same `kindredPeople` the norms read), but not "nothing to build" either. |
