...
| **Foresight & Crossroads on the Map** | **UNBLOCKED 2026-08-17 (D192)** — build it | none — it was never data | Both need `map-tab.jsx` — which is EAGER — to read a new store, and `MAX_EAGER_KB` had no headroom. It has 53 KB now: dropping the demo roster from shipping builds took the eager graph 964 → 925 against an unchanged 978 ceiling. D136 parked `paths.mapTree()` over **~1 KB**, and D-era labels were refused over **2 KB**, so this is ~25× the largest thing this budget has ever rejected. |
