...
| **Foresight CALL + its feed cards** | ~~Tier A only~~ **BUILT (D193)** | ~~a resolver that grades a call on our own aggregates~~ shipped: `resolveCallsV2` | D127 is unchanged by v28: the blocker was resolution integrity, not design. Tier A self-resolves on our own aggregates, and the card re-runs the grade on the device. The prototype's ten-second clock is the one part not ported. |
