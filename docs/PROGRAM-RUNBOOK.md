...
| --- | --- | --- | --- | --- |
| InSight PR shepherd | `55 */3 * * *`, bound to **Ops dispatcher B** — the row is *(B)*, created 2026-09-03 ~13:00. The original (`trig_01KZYMFk5gUQ1QSFbzhm71FD`, hourly, bound to the 564k-token dispatcher) is **disabled**, renamed *retired 2026-09-03 — 564k dispatcher*: an hourly fire on that binding measured $6.96 and 2.90M cache-read tokens. The planned `20 6,16 * * *` plus GitHub `pull_request` events was never built — the events need the Claude GitHub App, which is not installed | `claude-opus-5` | `OPS-RUNBOOK.md` §4, *The PR shepherd* | the door every approved PR ends at — create it first. The GitHub triggers need the Claude GitHub App installed; the web UI prompts for it |
