...
| One cold boot | **+250 reads** — the five surfaces fetched up front, not the bank | `BANK_SURFACES` in src/v2/data/live.ts (test · group · duo · pulse · call). **This row said "the whole question bank" until 2026-09-06**, and `check:figures` kept that number current as the bank grew — a gate faithfully maintaining a false sentence, which is how the belief that a phone downloads every question survived the work that stopped it |
| …and what it does not fetch | daily, feed and learn are **paged**, so the whole bank is 913 docs / 260.7 KiB of JSON and 663 of them never ride the boot | D383 (the daily's published length), `FEED_PAGE`, `LEARN_PAGE` — the three surfaces the content lanes grow every day are exactly the three that page |
