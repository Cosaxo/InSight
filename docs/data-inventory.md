...
| Test *item* answers | `v2_users/{uid}/answers/test-*` | owner only; counts published k-floored | The individual cards the results above are computed from. They ship as ordinary feed cards (`surface: "test"`), so their **option counts** publish like any other question's. The eighteen `test-political-*` items are the exception on the slicing side: they carry no per-anchor breakdown at all (D44), because a political item split by city and education is the Art. 9 exposure the row above rules out for the result vector. The other test families (big5, values, attachment) do slice — that is what the Mirror's cohort views read |
...
| Aggregates (public) | `v2_question_aggs` | any signed-in user | k-floored (≥5 by design; **paused to ≥1 pre-launch, D81** — counts publish exactly, per answer, and a tiny cohort's count is readable as an individual's answer until the floor restores), no per-vote timing; the once-per-5 publish cadence pauses with it |
