...
| Logic score histogram | `v2_logic_norms/global` (public mirror); `v2_logic_norms_private/global` (exact) | any signed-in user (mirror); nobody (exact) | 26 score buckets (0–25) + a count + the form length they belong to — no uid, no anchors, no timing. The `items` field starts a fresh era on any form-length change: 12-item scores never mix with 25-item ones (D59). Only the FIRST scored attempt per account counts (the D32 rule). The mirror materializes at or above the aggregate floor and updates once per 5 counts, the same step-attribution discipline as `v2_question_aggs`. Survives account deletion: the count was never attributable to begin with |
