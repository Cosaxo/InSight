...
| Core test results | `v2_users/{uid}.testResults` | any signed-in user | Big Five, politics, values, social. **Political results are special-category data under GDPR Art. 9** — they are never sliced by, never published, and never leave the owner doc |
