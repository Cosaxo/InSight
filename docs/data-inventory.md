...
| Logic score histogram + difficulty stats | `v2_logic_norms/{global,families}` (public mirrors); `v2_logic_norms_private/{global,families}` (exact) | any signed-in user (mirrors); nobody (exact) | 26 score buckets (0–25) + a count + the form length they belong to — no uid, no anchors, no timing. The `items` field starts a fresh era on any form-length change: 12-item scores never mix with 25-item ones (D61). Only the FIRST scored attempt per account counts (the D32 rule). The mirror materializes at or above the aggregate floor and updates once per 5 counts, the same step-attribution discipline as `v2_question_aggs`. Survives account deletion: the count was never attributable to begin with. The `families` doc (D62) is the difficulty ledger: per rule-family seen/solved counts and per-slot solve counts from the same first attempts — **no timings** (per-item timings never leave the device; difficulty is learned from solve rates alone) |
