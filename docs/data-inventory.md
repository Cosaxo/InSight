...
| Core test results | `v2_users/{uid}.testResults` | any signed-in user | Big Five, politics, values, social. Read across users since D107: the City constellation and its person card show a named person's axis scores beside yours — the person-to-person Compare read the D98 rules comment opened this field for (D98 also removed the Art. 9 carve-out this row used to assert — D44's reversal). What still never happens: results are not a breakdown dim, are never folded into any cohort aggregate, and cross-user readers must parse them defensively (`data/similarity.ts`) because the rules validate only the key set. Place score profiles are computed from test *item* answer counts (next rows), never from this field |
...
| Logic score histogram + difficulty stats | `v2_logic_norms/{global,families}` (public mirrors); `v2_logic_norms_private/{global,families}` (exact) | any signed-in user (mirrors); nobody (exact) | 26 score buckets (0–25) + a count + the form length they belong to — no uid, no anchors, no timing. The `items` field starts a fresh era on any form-length change: 12-item scores never mix with 25-item ones (D61). Only the FIRST scored attempt per account counts (the D32 rule). The mirror materializes at or above the aggregate floor and updates on the same cadence as `v2_question_aggs` (once per 5 counts by design; per count while D81's launch pause holds — the same step-attribution discipline, paused together). Survives account deletion: the count was never attributable to begin with. The `families` doc (D62) is the difficulty ledger: per rule-family seen/solved counts and per-slot solve counts from the same first attempts — **no timings** (per-item timings never leave the device; difficulty is learned from solve rates alone) |
...
| Aggregates (public) | `v2_question_aggs` | any signed-in user | exact (≥5 by design; **paused to ≥1 pre-launch, D81** — counts publish exactly, per answer, and a tiny cohort's count is readable as an individual's answer until the floor restores), no per-vote timing; the once-per-5 publish cadence pauses with it |
