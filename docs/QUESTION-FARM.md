...
A separate daily Routine (added 2026-07-30, alongside D14/D15) authors
**one** new catalog `pick` card per firing — a card in `window.PICK_QS`
(`src/v2/spec/pick-data.js`), the "favourite X from a shipped catalogue"
class. Same governance as the farm: this section is the contract, the PR
is the human gate, and every outcome logs to issue #31.
