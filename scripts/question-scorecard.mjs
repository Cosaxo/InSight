...
//   - FIT (2026-09-03, D439): the nightly Patterns fit's own numbers,
...
function summarize(card) {
  ...
  // Same optional-chaining rule for the fit block (D439), one step
  ...
  if (card.fit !== undefined) {
    ...
    console.log(
      f === null
        ? "  fit (D439): not published yet"
        : `  fit (D439): ${f.questions} questions · ${f.basis?.ready} at basis ≥ ${f.basis?.floor}` +
          (f.quality
...
if (FETCH || INPUT) {
  ...
  // The fit block (D439), merged BEFORE the card writes — the attention
