...
for (const q of entries) {
  ...
  if (q.type === "scale") {
    ...
    // CORE, since D437 (2026-09-10, the owner's word). This rule used to
    ...
    // fit reads picks since D434, and the owner expected every card to
    ...
    if (q.core !== true) {
      errors.push(`${q.id}: a catalog question is core (D437) — it ships whole to every device; the Mirror never folds it and its readers never see it, so the flag is the serving rule alone`);
    }
