...
// part of (D184's `rates`), and the types that scorecard can average. Both
...
// the class of failure D184 exists to close, reappearing one layer down.
  ...
  if (surface === "daily") {
    ...
    if (q.rates !== undefined) {
      if (!RATES_SCOPES.has(q.rates)) {
        err("rates", `rates ${JSON.stringify(q.rates)} — city|country|world (D184); anything else names no stop, so the question lands on no scorecard`);
      }
      if (!RATES_TYPES.has(q.type)) {
        err("rates", `a \`rates\` question is ${q.type} — the scorecard AVERAGES it (D184), and only rating|scale carry a magnitude to average`);
      }
    }
