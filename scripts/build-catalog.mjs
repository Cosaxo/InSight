...
ORDER BY DESC(?links) LIMIT 2000`,
  ...
  artists: {
    ...
    // played or wrote (D255 has the measurements). `refine` below is what
...
// few names in the artists domain are a human's call (D255): a reviewer
  ...
  try {
    ...
    console.error(
      `build-catalog: cannot reach ${ENDPOINT} — this is an operator step and needs\n` +
        ...
        `the environment's network policy as the thing that decides, and D255 the run\n` +
