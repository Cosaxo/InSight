...
//                     (the warm paint, D353) or off the network
//   LIVE.attached     the network boot completed this session; what
//                     `ready` meant before D353, and what re-entry keys on
...
async function getDb(): Promise<import("firebase/firestore").Firestore> {
  ...
  // NOTHING REACHES FIRESTORE WHILE THE UID IS THE MIRROR'S (D353). The
...
const state = {
  ...
  // D353: the WARM paint (everything below read off this device's own
  ...
  warm: false,
  // `uid` is the profile mirror's, not yet auth's (D353): the warm paint
...
// ── answers the server has not yet acknowledged, mirrored on disk (D354) ──
//
...
// boot (D353) made reachable. An answer written offline lives in the
...
// by the create-only rule. Before D353 an offline relaunch showed the
...
// hide, or (since D353, when the poll starts beside the boot's unguarded
...
async function refreshAggs(qids: readonly string[]): Promise<void> {
  ...
  try {
    ...
    snap.docs.forEach((d) => {
      ...
      // pending answer at boot (D354) — which this aggregate cannot hold
...
// last saw, and — since D353 — a mirror of the account's own profile. The
...
// changed, so a device that has booted once since D353 always paints
...
// ── the account's own profile, mirrored on this device (D353) ─────────
//
...
async function warmFromDisk(uid: string | null): Promise<DiskCaches> {
  ...
  // …and over them, the answers the server has not acknowledged (D354):
  ...
  // the pre-D353 boot: the feed globals in particular must not go out
...
async function hydrate(): Promise<void> {
  ...
  // ── the disk first, and the warm paint (D353) ──
  ...
  // takes, which is the trade D353 makes on purpose: the screen the lost
  ...
  if (!all) {
    ...
    // THE THREE RUN AT ONCE (D353). They read disjoint slices of one
  ...
  // about (D353) and awaited where the poll used to be started, past the
  ...
  if (uidA) {
    ...
    const fold = (
      ...
      if (val !== null) {
        ...
        // yet refuse — D354's second review, reasoned from the SDK's
    ...
    // SAME round trip as the answered delta (D353): the two queries share
    ...
    // beside the answered delta above (D353); this is where it folds.
    if (editPage) {
    ...
    if (answersFrom === "idb") {
      ...
      // …minus what is still INFLIGHT (D354): a restored pending answer is
    ...
    // then against the drained queue (D354). Not awaited: the boot has
  ...
  if (uid0) {
    try {
      ...
      // pre-D353 behaviour, restored after a review: the read was issued
...
const LIVE = {
  ...
  async flagAvatar(target: string): Promise<void> {
    ...
    notify();
    try {
      // Inside the try since D353: getDb() holds while the uid is the
  ...
  get bootError(): string {
    ...
    // (D353) started reading it on a live screen.
  ...
  // The network boot has completed this session (D353). `ready` can be
...
async function resubscribeForToday(): Promise<void> {
  // `attached` rather than `ready` (D353): before the attach the boot
...
export function refreshLive(): Promise<void> {
  ...
  refreshInFlight = (async () => {
    ...
    notify();
    // THE WARM PAINT DOES NOT WAIT FOR THE SDK EITHER (D353). anonSignIn is
    ...
    // now triggers is what D51 asks for and what the pre-D353 boot could
    ...
    try {
      ...
      // The deck poll starts INSIDE hydrate since D353 — the moment there
      ...
      // Before D353 the poll started after the answers block and this
    ...
    state.bootStage = "";
    state.ready = true;
    // The attach (D353): the network has been heard from this session. A
...
function wake(): void {
  ...
  // `attached`, not `ready` (D353): a warm-painted session whose network
...
// nobody issued (warm-boot.test.ts found it as a flake, D354).
...
export async function initLive(timeoutMs = 2500): Promise<void> {
  ...
  engagement.arm({
    ...
    // Confirmed, not provisional (D353): a rollup handed over while the
  ...
  subscribeToAuth((user) => {
    ...
    if (linkedChanged) notify();
    // While the uid is the mirror's (D353), a null state is the SDK
  ...
  // THREE runners, not two (D353). The warm paint — hydrate publishing a
  ...
  // `enabled` for D353's reason: a warm-painted session is enabled and can
