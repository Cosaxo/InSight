// The warm paint (D354): a returning device draws its real deck off its
...
// settles, and until D354 the only runners were the whole network boot
...
const h = vi.hoisted(() => ({
  ...
  // waitForPendingWrites: the SDK's queue-drained signal (D355). Parked
...
vi.mock("firebase/firestore", () => {
  ...
  const snapOf = (docs: FakeDoc[]) => ({
    ...
    docs: docs.map((d) => ({
      ...
      // server's copy (D355's second review) — a fake doc sets it with
      // `pending: true`.
...
// A device that has been through one full boot since D354: every store
...
// The SDK reports its queue drained (D355).
...
describe("the warm paint", () => {
  ...
  it("a first boot still waits for the network, and labels the wait", async () => {
    // Nothing on disk: the pre-D354 boot, unchanged. The deadline is what
  ...
  it("a cached bank without the profile mirror waits — an answer must never snapshot empty anchors", async () => {
    await seedBank([row("q_1", 1)], h.contentRev, 1000);
    await seedAnswers("uid_test", { q_1: "1" }, 500);
    // No mirror: a device on its first boot since D354.
    h.gated = true;
    const mod = await import("./live");
    const LIVE = mod.default;
    await mod.initLive(30);
  ...
  it("a changed contentRev replaces the warm deck with the server's bank", async () => {
    ...
    // as the full fetch takes, which is the trade D354 makes on purpose.
    expect(LIVE.ready).toBe(true);
...
describe("the provisional account", () => {
  ...
  it("a boot that fails past the deck poll's start does not leave the poll armed", async () => {
    ...
    // reads, D354) and the answers reads are the unguarded ones — so a
  ...
  it("a null auth state while the uid is provisional starts no second sign-in", async () => {
    ...
    // already started. Before D354 state.uid was null here and the branch
...
describe("answers the server has not acknowledged (D355)", () => {
  // Before D354 an offline relaunch showed the demo deck, which hid this
...
describe("the network phase's round trips", () => {
  // The queries a boot issues are unchanged; what D354 changed is which
  ...
  it("a warm boot: meta, then the delta, then the answers and the deck's aggregates together", async () => {
    ...
    // Before D354 this was three trips — answered, then edited, then the
