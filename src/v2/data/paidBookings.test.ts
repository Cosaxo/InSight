...
let store: typeof import("./paidBookings");
...
beforeEach(async () => {
  ...
  S.emit = null;
  vi.resetModules();
  store = await import("./paidBookings");
});

describe("an account change under a read in flight", () => {
  const ROW = { id: "r1", data: () => ({ uid: "u1", text: "A's booking" }) };

  it("does not commit the previous account's rows to the new one", async () => {
    S.docs = [ROW];
    let open = () => {};
    S.gate = new Promise<void>((r) => { open = r; });
    const inFlight = store.loadBookings();
    await tick();
    S.emit?.({ uid: "u2" });
    let woke = 0;
    store.subscribeBookings(() => { woke += 1; });
    open();
    await inFlight;

    expect(store.myBookings(), "the previous account's rows landed under the new one").toBeNull();
  ...
  it("still commits when nobody switched — the control", async () => {
    ...
    expect(store.myBookings()?.length, "a read that owns its account did not commit").toBe(1);
  });
