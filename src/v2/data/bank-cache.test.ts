...
const h = vi.hoisted(() => ({
  ...
  rankOrders: {} as Record<string, (
    ...
    // D382's daily SHAPE doc — a length, not an order — plus D383's
  ...
  return {
    ...
    getDocs: (q: { path?: string; cons?: Constraint[] }) => {
      ...
      if (!delta) {
        ...
        // …and D382's two: the deck's `seq in [positions]` and the Scores
...
describe("question-bank cache", () => {
  it("fetches the whole bank on a cold boot and records the cursor", async () => {
    await bootLive();
    // FOUR since D382: the boot surfaces, the feed's core (D321), the
    ...
    // fallback — the whole surface, exactly the pre-D382 fetch — so the
  ...
  it("pages a bank larger than one page, and every question arrives", async () => {
    ...
    // queries, and one for the daily's fallback (D382).
  ...
  it("does not stop one page early when the bank is an exact multiple of the page", async () => {
    ...
    // pages, and that is what is pinned. Six since D382's daily fallback.
    expect(bankFetches()).toBe(6);
    const paged = h.bankQueries.filter((q) => q.cons.some((c) => c.kind === "startAfter"));
    expect(paged).toHaveLength(2);
    // Every cursor-carrying page names a surface. Since D382 the loop this
  ...
  it("migrates a legacy localStorage cache into IndexedDB and retires the key", async () => {
    ...
    await bootLive();
    // STILL ONE. The daily's resolution runs on this path too (D382), and
  ...
  describe("the paged daily (D382)", () => {
    ...
    it("pages the Scores pool by id instead of querying the surface (D383)", async () => {
      // 200 dailies, 60 of them place questions. Before D383 the top-up
  ...
  it("fetches every boot surface, pulse and call included — and the paged two deliberately not", async () => {
    ...
    // THE DAILY LEFT THIS LIST AT D382 and did not become a paged surface
