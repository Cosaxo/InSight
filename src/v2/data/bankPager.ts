// bankPager.ts — which cards a device actually fetches (D320 for learn,
// D321 for the feed tail; both build D316 phases 2–3).
//
...
// (D319). This module is the arithmetic of that — which qids are needed —
...
// nothing the server learns (D317's phase 1 boundary: the fetch pattern
...
//
// …AND A TOP-UP FILLS TO A PAGE, NOT BY A PAGE (D389, ALGORITHM-REFLECTION
// §4.6's third item, D350's amendment). Until D389 a boot took the first
 ...
 *    topic (`held`, D389) up to `pageSize`. A device holding a page of
 *    fresh cards fetches nothing; one that answered them all is handed a
 *    whole page; a caller that passes no `held` gets the pre-D389 page.
 */
...
/**
 * D317 phase 1's whole effect on serving, in one function: per-topic
