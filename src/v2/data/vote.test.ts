...
// The answers and aggregate caches live in IndexedDB since D303
...
// cacheStore's purge clear (D303) under whichever store registered later.
...
// ({uid, votes, maxTs}) while the storage is rows + a meta row (D303).
...
beforeEach(() => {
  ...
  // there since D303, and a shared factory would leak one test's rows
  ...
  vi.stubGlobal("window", {
    ...
    // (D303) and a stub that only records the type would leave the
...
describe("vote() optimistic path (inflight vs unaggregated)", () => {
  ...
  // ── the coalesced agg cache (D64, rows since D303) ──────────────────
  //
  ...
  // per-row in IndexedDB now (D303), which keeps the same three ways to
  ...
  it("a uid change carries no previous account's aggregate past the purge", async () => {
    ...
    // never used to be. Since D303 the cancel is load-bearing where it
...
describe("LIVE.deleteAccount — the on-device half of erasure", () => {
  ...
  it("terminates the client and clears the IndexedDB cache, in that order", async () => {
    ...
    // Both boxes hold traces: a legacy localStorage blob and the D303
    ...
    // page's claim is about the device, not about a dispatch (D303).
