...
const h = vi.hoisted(() => ({
  ...
  // were asked for (D122). The learn prefetch's whole failure mode is
...
// The learn crowd split, warmed before the tap (D122).
//
...
describe("LIVE.loadLearnAggs — warming the split before the tap (D122)", () => {
  ...
  it("is what the reveal was missing — an unwarmed read is null however much data exists", async () => {
    ...
    // shipped behaviour up to D122, and the reason the authored estimate
