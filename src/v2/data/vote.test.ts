...
describe("vote() optimistic path (inflight vs unaggregated)", () => {
  ...
  // Learn left the boot fetch at D306 — a live device meets cards through
  ...
  it("publishes the bank's learn cards in the engine's own vocabulary", async () => {
    ...
    // pager is deliberately not part of boot (D306), so the page lands
    // just after ready.
    await vi.waitFor(() => {
...
describe("LIVE.learnMine — the answer the trigger has not folded yet", () => {
  const CARD = {
    ...
    data: {
      ...
      // Keyed since D306: cardLanded() watches the engine pool, and the
  ...
  // Learn pages since D306: the card reaches state.learnBank through the
