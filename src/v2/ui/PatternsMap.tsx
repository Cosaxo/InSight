// The Map lens (D215, redrawn 2026-09-02; every kind of dot since D461) —
...
/** The most dots one ring draws (D458): a rim of radius 131 is about 820
...
/** The pseudo-topic that rings the viewer's own answers (D458). */
export const MAP_TOPIC_ANSWERED = "answered";
/** The You arc's radius — inside the rim, clear of the hub (D461). The
...
/** The hairline between two bead groups inside one topic (D461). */
 ...
 *
 * A QUESTION'S BEADS STAY TOGETHER (D461). Rows carry their `qid`, and
...
export default function PatternsMap({ items, version, topic, guide = false }: {
  ...
  // THE YOU ARC IS OFF BY DEFAULT (D461, the owner's yes of 2026-09-10).
  ...
  // a chosen topic re-rings the field (D458), so a selection made on one
  // ring does not carry to the next
  React.useEffect(() => { setSelQ(null); }, [topic]);

  // THE RING IS THE TOPIC'S OWN (D458). The chip used to dim the other
  ...
  const geo = React.useMemo(() => {
    ...
    if (inRing.length > MAP_DOT_BUDGET) {
      ...
      // A GROUP IS TRIMMED AS A GROUP (D461). The quota is blind to bead
  ...
  // THE YOU ARC (D461, the owner's yes). The anchor rows the fit folds
  // (D455) are not questions and do not belong on a rim of questions, so
  ...
  // topic's since D458, so a question off the ring is no selection at all.
  ...
  // throwing. (`inTopic` went with D458: the ring holds the topic's own
  ...
  // A CATALOGUE'S NAMES ARE FETCHED ON THE TAP, never for the rim (D461):
  ...
  return (
    <>
      <div className="ln-card">
        ...
        {/* the You toggle (D461): off by default, and absent entirely on a
          ...
          <svg className="ln-svg" viewBox={`0 0 ${S} ${S}`} role="img"
            ...
            <g>
              {RG.pts.map((p) => {
                ...
                // at rest nothing dims: the ring is the topic's own (D458)
                ...
                return (
                  <g key={D[i].key} className="qm-dot"
                    ...
                    {/* the rest of a trimmed group, folded into a count at
                        its own place (D461) — the bead that survived is the
            ...
            {/* the hairlines between one question's beads and the next's
                (D461) — a tick, not a gap: a gap is what parts TOPICS, and
            ...
            {/* the You arc, inside the rim and only while it is on (D461) */}
            {you.length > 0 && (
              <g className="qm-ink">
      ...
      {q && sel != null && selHue != null ? (
        <div className="qm-read">
          ...
          {/* THE CHIP ROW (D461). A question with several answers first has
            ...
            {says && says.id === q.key && says.rows.map(({ j, s }, k) => {
              ...
              return (
                <div className="qm-say" key={k}>
                  {/* THE SENTENCE IS THE SAME SHAPE FOR EVERY KIND (D461),
                  ...
                  {/* the stated basis — the D146 rule the prototype had no
                      data to need, and since D461 it also says WHICH
