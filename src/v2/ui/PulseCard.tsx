// The daily pulse, compact, beside the blind daily — the v24 design
// (design/standalone-v24/pulse-card.jsx) ported typed. Same contract as
// the daily question: you answer before you see anyone else; the five
// inputs BECOME the chart on reveal. One hue throughout (--pulse).
// "Your line →" opens the Trends reading in place — the chart is a lazy
// chunk (check:bundle: the card is first-screen, the reading is not).
import React from "react";
import PULSE from "../data/pulse";

const PulseTrendsLazy = React.lazy(() => import("./PulseTrends"));

export default function PulseCard(): React.ReactElement | null {
  const [, bump] = React.useState(0);
  const [open, setOpen] = React.useState(false);
  React.useEffect(() => {
    void PULSE.ensureLive().catch(() => { /* the ask renders without a crowd */ });
  ...
  const mine = PULSE.mineToday();
  const st = PULSE.streak();
  const nToday = PULSE.todayN("world");
  const bins = PULSE.bins("world");
  ...
  // answered, faint = missed, ring = today still open. Tap → the reading.
  const strip = (
  ...
  const ask = (
    <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
      <div style={{ display: "flex", gap: 6 }}>
        {steps.map((s, i) => (
          <button key={s.v} className="press" onClick={() => PULSE.answer(s.v)} aria-label={s.label}
    ...
    <div className="card" data-screen-label="Daily pulse" style={{ display: "flex", flexDirection: "column", gap: 11, padding: "13px 14px 14px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <span style={{ display: "flex", alignItems: "center", gap: 7 }}>
          ...
          <span className="kicker" style={{ marginBottom: 0 }}>{PULSE.Q.kicker}</span>
        </span>
        {strip}
      </div>
      <div style={{ fontFamily: "var(--sans)", fontWeight: 800, fontSize: 21, lineHeight: 1.12, letterSpacing: "-0.03em", textWrap: "balance" }}>{PULSE.Q.text}</div>
      {mine == null ? ask : reveal}
      {open && (
        <div style={{ borderTop: "1px solid color-mix(in oklch, var(--rule), transparent 30%)", paddingTop: 12 }}>
          <React.Suspense fallback={null}>
            <PulseTrendsLazy compact />
          </React.Suspense>
        </div>
      )}
    </div>
