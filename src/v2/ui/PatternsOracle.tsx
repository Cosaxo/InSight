...
// this many graded answers with a base rate (D454) — the server
...
function OrDone({ log, qOf, anyOpen, skill }: {
  ...
  qOf: (qid: string) => PoolItem | undefined;
  anyOpen: boolean;
  /** The meter's verdict on this viewer (D454), printed with its basis
   * once it has one. */
  skill?: string | null;
}): React.ReactElement {
      ...
      <div key={q.id} className="ln-card fade-in"
            ...
            {work === "pending" ? (
              ...
              return (
                <>
                  ...
                  {/* the groups that carried the call (D451): the viewer's
        ...
        <div style={{ flex: "none", marginTop: 12, paddingTop: 10, borderTop: "1px solid var(--rule)" }}>
          <div className="pt-kick">
            ...
            {/* the verdict, once the record has a basis (D454): outside
