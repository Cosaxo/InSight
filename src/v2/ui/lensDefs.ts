...
export interface LensQuestion {
  ...
  branch?: string;
  /**
   * The bank's short label — "Nature access", "Getting around" (D184).
   *
   ...
   * a pre-D184 seed, so the card falls back to the prompt.
   */
  tag?: string;
  /**
   * Which Mirror stop's scorecard may fold this question — "city" |
   * "country" | "world" (D184), absent on every question that rates no
   * place, which is most of the bank.
   *
   * THE POINT OF THE FIELD is that a place scorecard has to be about the
   * place. Until D184 Scores drew every ordinal question the archive
