...
// re-ported at D432 from design/standalone-2026-09-09/group-role-map.jsx:
...
// group-role-map.jsx — the group as a cast list. Role votes ("bank heist:
// who drives?") name members into ROLES; here every member (you included)
// is a star with their held roles orbiting as satellites, coloured by
// scenario pack — like the You map, but the orbits are the room's votes
// about each person. Contested roles sit between their two rivals. Tap a
// role for the vote behind it; tap a person for their role-sheet, with
// their seat said as its line. The live twin is ui/LgRoleMap.tsx over
// data/roleField.ts; the geometry is the same.
const EXPORTS = {};
(function () {
  ...
  function buildField(gid) {
    ...
    rv.roles.forEach((role) => {
      if (!byId[role.winner]) return;
      if (role.contested && byId[role.second]) {
    ...
    // satellites: each person's held roles fan out on the arc facing AWAY from
    // the field's centre, so orbits never reach into the middle of the map
    people.forEach((p) => {
    ...
    // relax: satellites apart from each other and clear of every star
  ...
  function VoteDots({ count, hue }) {
    return (
      <span style={{ display: 'flex', gap: 3.5, alignItems: 'center', flexShrink: 0 }}>
        {Array.from({ length: count }, (_, i) => <span key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: hueDot(hue) }}></span>)}
      </span>
    );
  }
  function Av({ person, size = 26 }) {
    if (person.me) {
      return <span style={{ width: size, height: size, borderRadius: '50%', background: 'var(--ink)', color: 'var(--surface)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--sans)', fontSize: size * 0.34, fontWeight: 700, flexShrink: 0 }}>you</span>;
    }
    return <GDAv p={person.p} size={size}></GDAv>;
  }
  function ScenChip({ scen }) {
    return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 9px', borderRadius: 999, background: `oklch(0.95 0.03 ${scen.hue})`, fontFamily: 'var(--sans)', fontSize: 10.5, fontWeight: 700, color: `oklch(0.45 0.1 ${scen.hue})`, whiteSpace: 'nowrap', flexShrink: 0 }}>{scen.label}</span>;
  }

  // the vote behind a role: who was named, by how many
  function RoleVoteCard({ field, role }) {
    const rows = field.people.map((p) => ({ p, count: role.votes[p.id] || 0 })).filter((r) => r.count > 0).sort((a, b) => b.count - a.count);
    return (
      <div className="card" style={{ marginTop: 12, padding: '15px 16px' }}>
        ...
        <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--ink-3)', marginBottom: 12 }}>
          {role.prompt}{role.r ? ' · round ' + role.r : ''}{role.contested ? ' · contested' : ''}
        </div>
  ...
  // a person's role-sheet: their seat, then every role the room gave them
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: all.length ? 13 : 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontFamily: 'var(--sans)', fontSize: 11.5, color: 'var(--ink-3)' }}>
              {person.seat ? person.seat.line : all.length ? `how ${gname} cast ${person.me ? 'you' : person.name}` : `${gname} hasn't cast ${person.me ? 'you' : person.name} yet`}
            </div>
          </div>
          {!person.me && NAV.can('openPerson') && (
            <button className="press" onClick={() => NAV.openPerson(person.id)} style={{ cursor: 'pointer', WebkitAppearance: 'none', border: '0.5px solid var(--rule)', background: 'var(--surface)', borderRadius: 999, padding: '6px 12px', minHeight: 44, fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 600, color: 'var(--ink-2)', whiteSpace: 'nowrap' }}>profile →</button>
          )}
        </div>
        {all.length > 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
  ...
  function GroupRoleMap({ gid, gname }) {
    ...
    const roleDim = (role) => { if (!sel) return false; if (selRole) return selRole.key !== role.key; return !(role.holderIds || []).includes(selPerson.id); };
    const personDim = (p) => { if (!sel) return false; if (selPerson) return selPerson.id !== p.id; return !(selRole.holderIds || []).includes(p.id); };
    ...
    // at 360×336 and the viewBox grows vertically to keep the ring round
    ...
    return (
      <div className="mf-flex">
        <div ref={wrapRef} className="mf-canvaswrap" style={{ margin: '4px -6px 0' }}>
          ...
          <svg viewBox={`0 ${vbY} ${W} ${vbH}`} preserveAspectRatio="xMidYMid meet" role="group" aria-label={`${gname} as a cast`}
            ...
            {field.sats.map((role, i) => {
              ...
              const pick = () => setSel(on ? null : { kind: 'role', key: role.key });
              return (
                <g key={role.key} role="button" tabIndex={0} aria-pressed={!!on}
                  aria-label={`${role.label} · ${role.holderIds.map((id) => field.byId[id].name).join(' and ')}${role.contested ? ' · contested' : ''}`}
                  onClick={pick} onKeyDown={onKey(pick)}
                  style={{ cursor: 'pointer', outline: 'none', opacity: roleDim(role) ? 0.15 : 1, transition: 'opacity 0.25s ease', transformBox: 'fill-box', transformOrigin: 'center',
                    '--dx': (d1 * 5 - 2.5).toFixed(1) + 'px', '--dy': (d2 * 5 - 2.5).toFixed(1) + 'px',
            ...
            {field.people.map((p, i) => {
              ...
              const rr = p.rr;
              const pick = () => setSel(on ? null : { kind: 'person', id: p.id });
              return (
                <g key={p.id} role="button" tabIndex={0} aria-pressed={!!on} aria-label={p.name + (p.top ? ' · ' + p.top.label : '')}
                  onClick={pick} onKeyDown={onKey(pick)}
                  style={{ cursor: 'pointer', outline: 'none', opacity: personDim(p) ? 0.25 : 1, transition: 'opacity 0.25s ease', animation: `grIn 0.45s ${0.08 + i * 0.05}s both` }}>
