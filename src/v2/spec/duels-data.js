...
// the bank would give it, so the pool arithmetic (offsets, the depth
// ladder's wrap) is unchanged; only the pools are shorter.
...
(function () {
  ...
  // can never drift (single source, the D32 shape); what arrives here is
  // its generated sample (D430, the import above). Array order is the
  // rotation order and is deliberately interleaved: append, never sort —
  // and the sample keeps bank order, so the interleaving survives it.
  ...
  // sampled per domain for the demo (D430) — the sample takes the light end
  // of each domain, so the ladder is shorter here and duoQ's wrap does the
  // rest. JSON entries carry an `id` the live seed keys on; unused here —
  // demo duo state keys on the partner, not the question. The light → deep
