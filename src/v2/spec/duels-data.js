...
// and all), and re-ported at D432 from design/standalone-2026-09-09/
...
// The DEMO SAMPLE, not the bank (D430 — D284's shape one bank over). This
...
// The four seats, with their lines (D432) — the same list the live
...
// duels-data.js — the "know each other" layer, as ROUNDS (D426, D432).
...
(function () {
  ...
  // ── THE BANK: the D430 sample of content/duel-questions.json ───────────────
  // The group as a CAST (D429/D432): a role vote is a `pick` naming its
  ...
  // a member's seat: the share of the votes they received per seat (D432)
  ...
  // for the demo (D430) — the sample takes the light end of each domain, so
  // the ladder is shorter here and duoQ's wrap does the rest. The CAST entry
  // of each pool (D432) is the fourth round's question, not part of the
  // ladder.
  ...
  // The cast sets (D432): one entry per pool — "Most days, {name} is…" with
