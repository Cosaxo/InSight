...
prints the current total on every run. The count today is **379 across 42
...
386 → 379, and the interesting part is not the seven. Every conversion
...
the last file reading `window.TypeMark` and `window.TypeIndexSheet`, both
defined in `type-marks.jsx` (which already exported `TypeMark`;
`TypeIndexSheet` needed the keyword). No cycle — checked against the import
graph, not assumed, per the paragraph above: `type-marks.jsx` reaches 33
modules and `result-card.jsx` is not among them.
