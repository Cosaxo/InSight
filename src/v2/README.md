...
- `data/` and `ui/` — the typed layer. `data/live.ts` is the live store;
  ...
  imports the binding since D352), and until D352's sweep the two `ui/`
...
### `LIVE` — the store itself leaves the bridge (D352)
...
- **A publication a test reads is a publication with a reader.** Five
  ...
  the same lesson `learn-split` taught in the D352 section.
